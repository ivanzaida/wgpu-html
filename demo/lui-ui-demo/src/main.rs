...
impl Component for DemoApp {
  ...
  fn view(&self, _props: &DemoProps, ctx: &Ctx<DemoMsg>) -> El {
    el::div().class(ctx.scoped("root")).children([
      el::div().class(ctx.scoped("title")).text("lui-ui Demo"),
      ctx.child::<TextInput>(()),
...
struct UiDemoApp {
  mount: Mount<DemoApp>,
  tree: Tree,
  devtools: lui_devtools::Devtools,
...
fn main() {
  ...
  let devtools = lui_devtools::Devtools::attach(&mut tree, false);
