...
pub fn setup(world: &mut World) {
    let devtools = {
        let mut overlay = world.get_non_send_resource_mut::<HtmlOverlay>().expect("HtmlOverlay must exist — add LuiPlugin before this system");
...
pub fn input_system(
    ...
    for event in cursor_events.read() {
        ...
        if let Some(layout) = d.pipeline_cache.layout() {
            lui::interactivity::pointer_move(
                d.devtools.tree_mut(),
                layout,
