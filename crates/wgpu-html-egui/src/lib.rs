...
use egui::{
    Align2, Color32, Event, FontId, PointerButton, Pos2, Rect as EguiRect, Response, Sense, Ui,
    Vec2,
};
use wgpu_html::PipelineTimings;
use wgpu_html::interactivity;
use wgpu_html::layout::LayoutBox;
use wgpu_html::renderer::{DisplayList, Rect};
...
mod tests {
    ...
    fn pointer_buttons_map_to_tree_buttons() {
        ...
        assert!(matches!(
            pointer_button(PointerButton::Extra1),
            MouseButton::Other(3)
        ));
        assert!(matches!(
            pointer_button(PointerButton::Extra2),
            MouseButton::Other(4)
        ));
    }
