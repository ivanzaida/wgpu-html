...
use wgpu_html::{PipelineAction, PipelineCache, classify_frame, paint_tree_cached};
use wgpu_html_tree::{self, Tree};

fn parse_and_register_fonts(html: &str) -> Tree {
  let mut tree = wgpu_html::parser::parse(html);
...
fn typing_in_input_classifies_as_layout_only_not_full_pipeline() {
  ...
  wgpu_html_tree::focus(&mut tree, Some(&[0]));
  wgpu_html_tree::text_input(&mut tree, "a");
...
fn typing_does_not_bump_cascade_generation() {
  ...
  wgpu_html_tree::focus(&mut tree, Some(&[0]));
  wgpu_html_tree::text_input(&mut tree, "hello");
...
fn typing_pushes_dirty_path() {
  ...
  let input_path: Vec<usize> = {
    ...
    fn find_input(node: &wgpu_html_tree::Node, path: &mut Vec<usize>) -> bool {
      if matches!(&node.element, wgpu_html_tree::Element::Input(_)) {
        return true;
      }
  ...
  wgpu_html_tree::focus(&mut tree, Some(&input_path));
...
fn second_keystroke_also_classifies_as_layout_only() {
  ...
  // Second keystroke.
  wgpu_html_tree::text_input(&mut tree, "b");
  let action = classify_frame(&tree, &cache, &mut image_cache, 800.0, 600.0, 1.0);
  assert_eq!(action, PipelineAction::LayoutOnly,
...
fn form_with_image_input_does_not_force_full_pipeline_on_typing() {
  ...
  wgpu_html_tree::focus(&mut tree, Some(&[0, 1]));
  wgpu_html_tree::text_input(&mut tree, "x");
...
fn checkbox_toggle_classifies_as_patch_form_controls() {
  ...
  // Toggle checkbox.
  wgpu_html_tree::focus(&mut tree, Some(&[0]));
  tree.dispatch_mouse_down(Some(&[0]), (5.0, 5.0), wgpu_html_tree::MouseButton::Primary, None);
  tree.dispatch_mouse_up(Some(&[0]), (5.0, 5.0), wgpu_html_tree::MouseButton::Primary, None);
