...
fn file_input_has_form_control() {
  ...
  let mut images = wgpu_html_layout::ImageCache::default();
  let layout = wgpu_html::compute_layout(&tree, &mut text_ctx, &mut images, 800.0, 600.0, 1.0);
  let root = layout.unwrap();
  let path = input_path(&tree);
  let lb = wgpu_html::layout_at_path(&root, &path).unwrap();
  assert!(matches!(
    lb.form_control.as_ref().map(|fc| &fc.kind),
    Some(wgpu_html_layout::FormControlKind::File { .. })
  ));
}
...
fn file_button_width_includes_padding() {
  ...
  let btn_w = wgpu_html::paint::file_button_width(lb);
