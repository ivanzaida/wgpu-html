...
fn set_border_all(s: &mut wgpu_html_models::Style, w: f32, color: CssColor) {
...
fn set_radius_all(s: &mut wgpu_html_models::Style, r: f32) {
...
fn build_date_picker_tree(dp: &DatePickerState, tree: &Tree) -> CascadedTree {
  ...
  let mut header_s = wgpu_html_models::Style::default();
  ...
  let mut nav_s = wgpu_html_models::Style::default();
  ...
  let mut title_s = wgpu_html_models::Style::default();
  ...
  // ── Weekday header row ──
  let mut wd_row_s = wgpu_html_models::Style::default();
  ...
  let wd_children: Vec<CascadedNode> = (0..7u8).map(|i| {
    ...
    let mut cell_s = wgpu_html_models::Style::default();
  ...
  for row in 0..6u8 {
    let mut row_s = wgpu_html_models::Style::default();
  ...
  let mut grid_s = wgpu_html_models::Style::default();
  ...
  let time_row = if dp.has_time {
    let mut tr_s = wgpu_html_models::Style::default();
    ...
    let make_time_field = |val: u8| -> CascadedNode {
      let mut fs = wgpu_html_models::Style::default();
    ...
    let colon = cn(Element::Span(Span::default()), wgpu_html_models::Style::default(), vec![
  ...
  // ── Reset button ──
  let mut rb_s = wgpu_html_models::Style::default();
...
fn resolve_dim_color(dp: &DatePickerState) -> [f32; 4] {
  dp.calendar_style.as_ref()
    ...
    .and_then(|c| wgpu_html_layout::color::resolve_color(c))
    .unwrap_or([0.45, 0.45, 0.45, 1.0])
}

fn paint_chevron(list: &mut DisplayList, br: &wgpu_html_layout::Rect, color: [f32; 4], left: bool) {
