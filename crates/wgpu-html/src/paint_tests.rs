  ...
  fn assert_glyphs_fit_clips(list: &wgpu_html_renderer::DisplayList) {
    for cmd in &list.commands {
      if cmd.kind != wgpu_html_renderer::DisplayCommandKind::Glyph {
        continue;
      }
  ...
  fn simple_body_with_glyphs() -> LayoutBox {
    ...
    LayoutBox {
      ...
      text_run: Some(wgpu_html_text::ShapedRun {
        glyphs: vec![
          wgpu_html_text::PositionedGlyph {
            x: 0.0, y: 0.0, w: 10.0, h: 16.0,
            uv_min: [0.0, 0.0], uv_max: [1.0, 1.0],
            color: [1.0, 1.0, 1.0, 1.0],
          },
          wgpu_html_text::PositionedGlyph {
            x: 10.0, y: 2.0, w: 12.0, h: 20.0,
            uv_min: [0.0, 0.0], uv_max: [1.0, 1.0],
            color: [1.0, 1.0, 1.0, 1.0],
          },
        ],
        lines: vec![wgpu_html_text::ShapedLine {
          top: 0.0, height: 24.0, glyph_range: (0, 2),
        }],
        glyph_chars: vec![],
        text: "ab".to_string(),
