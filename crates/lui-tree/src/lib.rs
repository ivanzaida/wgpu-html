...
//! `lui-models`, or raw text) with its child nodes.
//!
//! Models stay pure data. Composition lives here.

use std::{collections::HashMap, ops::Range, sync::Arc, time::Duration};

use lui_models::{self as m, ArcStr};
...
pub use dispatch::{
  ...
  set_range_value_by_fraction, text_input, wheel_event,
};
pub use events::{
...
pub struct Tree {
  ...
  /// so that [`lui::PipelineAction::LayoutOnly`] can skip the
  /// full CSS cascade.
                ...
                match self {
                    ...
                    $(Element::$v(e) => match name {
                        ...
                        "dir" => e.dir.as_ref().map(|d| {
                            use lui_models::common::html_enums::HtmlDirection;
                            match d {
...
/// results, so [`lui::PipelineAction::LayoutOnly`] can skip
/// re-cascading.
