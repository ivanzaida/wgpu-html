...
//! ## Layers
//!
//! 1. **[`Driver`]** — trait with 5 mandatory methods. Implement it to
//!    connect wgpu-html to any windowing framework.
//!
//! 2. **[`Runtime`]** — struct parameterized over `D: Driver` that owns
//!    the renderer, text context, image cache, pipeline cache, and all
//!    cross-cutting state (scroll, cursor, clicks, scrollbar drag). Its
//!    event methods translate platform-agnostic data into engine
//!    operations and return whether a redraw is needed.
//!
//! ## Platform crates
//!
//! | Crate | Integration |
...
use wgpu_html::{
    ...
    scroll::{
        ElementScrollbarDrag, clamp_scroll_y, paint_viewport_scrollbar,
        rect_contains, scroll_element_at, scroll_y_from_thumb_top,
...
impl<D: Driver> Runtime<D> {
                ...
                match &drag {
                    ScrollbarDrag::Viewport { grab_offset_y } => {
                        self.scroll_y = scroll_y_from_thumb_top(
                            y - grab_offset_y, layout, w as f32, h as f32,
                        );
                    }
    ...
    fn start_scrollbar_drag(&mut self, tree: &mut Tree, pos: (f32, f32)) -> bool {
        ...
        if rect_contains(geom.track, pos) {
            ...
            self.scroll_y =
                scroll_y_from_thumb_top(thumb_top, layout, w as f32, h as f32);
