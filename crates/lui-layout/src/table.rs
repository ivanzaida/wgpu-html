...
fn resolve_border_spacing(style: &lui_cascade::ComputedStyle) -> Spacing {
  match style.border_spacing {
    ...
    Some(CssValue::Number(n)) => Spacing {
      h: *n as f32,
      v: *n as f32,
    },
    Some(CssValue::String(s)) | Some(CssValue::Unknown(s)) => {
      let parts: Vec<&str> = s.split_whitespace().collect();
      let parse_px = |s: &str| -> f32 { s.trim_end_matches("px").parse::<f32>().unwrap_or(0.0) };
      match parts.len() {
        1 => {
...
fn build_grid(row_infos: &[RowInfo], num_cols: usize) -> (Vec<Vec<bool>>, Vec<Vec<usize>>) {
  ...
  let mut start_cols: Vec<Vec<usize>> = row_infos.iter().map(|info| vec![0; info.num_cells]).collect();
...
pub fn layout_table<'a>(
  ...
  let table_width = sizes::resolve_length(b.style.width, ctx.containing_width).unwrap_or(available.max(0.0));
  ...
  for (ri, info) in row_infos.iter().enumerate() {
    ...
    for ci in 0..info.num_cells {
      ...
      if rs > 1 {
        ...
        let spanned: f32 = (ri..ri + rs).map(|r| row_heights[r]).sum::<f32>() + sp_v * (rs as f32 - 1.0);
        if needed > spanned {
