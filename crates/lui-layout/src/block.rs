...
use crate::{
  box_tree::{BoxKind, LayoutBox, Overflow, ScrollInfo},
  context::LayoutContext,
  geometry::Point,
  positioned, sides, sizes,
  text::TextContext,
};
...
fn parse_scrollbar_gutter(v: Option<&lui_core::CssValue>) -> ScrollbarGutter {
  let raw = css_str(v);
  ScrollbarGutter {
    stable: raw.split_ascii_whitespace().any(|token| token == "stable"),
    both_edges: raw.split_ascii_whitespace().any(|token| token == "both-edges"),
  }
}

fn reserves_stable_scrollbar_gutter(overflow: Overflow, gutter: ScrollbarGutter, scrollbar_width: f32) -> bool {
  gutter.stable && scrollbar_width > 0.0 && !matches!(overflow, Overflow::Visible | Overflow::Clip)
}
...
pub fn layout_block<'a>(
  ...
  let max_w = sizes::resolve_length_ctx(b.style.max_width, ctx.containing_width, &self_ctx).map(|v| {
    if is_border_box(b.style) {
      (v - frame_h).max(0.0)
    } else {
      v
  ...
  let reserve_y_gutter = has_y_scrollbar || reserves_stable_scrollbar_gutter(ov_y, scrollbar_gutter, scrollbar_w);
  let reserve_x_gutter = has_x_scrollbar || reserves_stable_scrollbar_gutter(ov_x, scrollbar_gutter, scrollbar_w);
  let y_scrollbar_reduction = if reserve_y_gutter {
    scrollbar_w * if scrollbar_gutter.both_edges { 2.0 } else { 1.0 }
  } else {
    0.0
  };
  let x_scrollbar_reduction = if reserve_x_gutter {
    scrollbar_w * if scrollbar_gutter.both_edges { 2.0 } else { 1.0 }
  } else {
    0.0
  };

  // `scrollbar-gutter: stable both-edges` reserves a mirrored gutter on the start edge.
  if reserve_y_gutter && scrollbar_gutter.both_edges {
    b.content.x += scrollbar_w;
  }
  if reserve_x_gutter && scrollbar_gutter.both_edges {
    b.content.y += scrollbar_w;
  }

  // Reduce content area for visible scrollbars and stable gutters.
  ...
  for child in b.children.iter_mut() {
    ...
    if float_val == "left" || float_val == "right" {
      ...
      let old = std::mem::replace(child, placeholder);
      let mut result = crate::engine::layout_node(
        old,
        &child_ctx,
  ...
  let max_h = sizes::resolve_length_ctx(b.style.max_height, ctx.containing_height, &self_ctx).map(|v| {
    if is_border_box(b.style) {
      (v - frame_v).max(0.0)
    } else {
      v
  ...
  // set clip rect.
  let is_scroll_container = !matches!(ov_x, Overflow::Visible) || !matches!(ov_y, Overflow::Visible);
  ...
  if is_scroll_container {
    ...
    let show_y_scrollbar = has_y_scrollbar || (ov_y == Overflow::Auto && overflows_y);
    let show_x_scrollbar = has_x_scrollbar || (ov_x == Overflow::Auto && overflows_x);

    // If auto scrollbar now appears, reduce content area retroactively.
    // This is a simplification — browsers do a second layout pass.
    if show_y_scrollbar && !reserve_y_gutter {
      b.content.width = (b.content.width - scrollbar_w).max(0.0);
    }
    if show_x_scrollbar && !reserve_x_gutter {
      b.content.height = (b.content.height - scrollbar_w).max(0.0);
    }

    let sb_w = if show_y_scrollbar || show_x_scrollbar {
      scrollbar_w
    } else {
      0.0
    };
...
fn is_bfc_root(style: &lui_cascade::ComputedStyle) -> bool {
  ...
  matches!(overflow_x, "hidden" | "auto" | "scroll" | "clip")
    ...
    || matches!(
      display,
      "flex" | "inline-flex" | "grid" | "inline-grid" | "inline-block" | "flow-root"
    )
...
pub(crate) fn apply_text_align(child: &mut LayoutBox, text_align: &str, container_width: f32) {
  ...
  if !matches!(child.kind, BoxKind::AnonymousBlock | BoxKind::AnonymousInline) {
    return;
  }
  ...
  match text_align {
    "center" => {
      if free > 0.001 {
        positioned::translate_recursive_pub(child, free / 2.0, 0.0);
      }
    }
    "right" | "end" => {
      if free > 0.001 {
        positioned::translate_recursive_pub(child, free, 0.0);
      }
    }
    "justify" => {
