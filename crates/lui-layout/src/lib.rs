...
use lui_style::{CascadedNode, CascadedTree, PseudoElementStyle};
use lui_text::{ParagraphSpan, PositionedGlyph, ShapedLine, ShapedRun, TextContext};
use lui_tree::{Element, Node, ScrollOffset, TextCursor, Tree};
...
fn compute_bg_tiles(
  ...
  off_x: f32,
  off_y: f32,
  repeat: lui_models::common::css_enums::BackgroundRepeat,
) -> Vec<Rect> {
  use lui_models::common::css_enums::BackgroundRepeat as BR;
...
pub struct FileButtonStyle {
  ...
  pub padding: [f32; 4],
  pub cursor: Cursor,
  pub text_run: Option<lui_text::ShapedRun>,
}
...
fn needs_full_relayout(node: &CascadedNode) -> bool {
  use lui_models::common::css_enums::FlexDirection;
    ...
    if is_dirty {
      ...
      if needs_full_relayout(child_node) {
        ...
        *child_box = layout_block(
        ...
        let new_h = child_box.margin_rect.h;
        let child_position = child_node.style.position.clone().unwrap_or(lui_models::common::css_enums::Position::Static);
        if !is_out_of_flow_position(child_position) {
          cursor_dy += new_h - old_h;
        }
      } else {
...
fn resolve_lui_color(
  custom_properties: &std::collections::HashMap<lui_models::ArcStr, lui_models::ArcStr>,
  name: &str,
  current: Color,
) -> Option<Color> {
...
fn resolve_lui_properties(
  cp: &std::collections::HashMap<lui_models::ArcStr, lui_models::ArcStr>,
  fg: Color,
) -> LuiProperties {
  LuiProperties {
...
fn lui_popup_from_pseudo(node: &CascadedNode) -> Option<std::sync::Arc<lui_models::LuiPopupStyle>> {
  let s = node.lui_style(lui_tree::PseudoElement::LuiPopup)?;
  Some(std::sync::Arc::new(lui_models::LuiPopupStyle {
    width: s.width.clone(),
...
fn lui_color_from_pseudo(node: &CascadedNode) -> Option<std::sync::Arc<lui_models::LuiColorPickerStyle>> {
  use lui_tree::PseudoElement;
  let mut p = lui_models::LuiColorPickerStyle::default();
  let mut any = false;
...
fn lui_calendar_from_pseudo(node: &CascadedNode) -> Option<std::sync::Arc<lui_models::LuiCalendarStyle>> {
  use lui_tree::PseudoElement;
  let mut p = lui_models::LuiCalendarStyle::default();
  let mut any = false;
...
fn resolve_length_px(val: Option<&lui_models::common::CssLength>, font_size: f32) -> Option<f32> {
  use lui_models::common::CssLength;
  match val? {
...
fn resolve_border_radius_single(val: Option<&lui_models::common::CssLength>) -> Option<f32> {
  ...
  if !wraps_multiline {
    if let Some(run) = run.as_mut() {
      ...
      if matches!(
        ...
        Some(lui_models::common::css_enums::TextAlign::Center)
      ) {
...
fn form_control_info_from_element(element: &Element) -> Option<FormControlInfo> {
  ...
  let kind = match inp.r#type {
    ...
    Some(InputType::DatetimeLocal) => {
      let val = inp.value.as_deref().unwrap_or("");
      let (y, m, d, hour, minute) = lui_tree::date::parse_datetime_local(val).unwrap_or((0, 0, 0, 0, 0));
...
fn letter_spacing_px(style: &Style, font_size: f32) -> f32 {
  use lui_models::common::css_enums::CssLength;
...
fn apply_text_transform(text: &str, tt: Option<&lui_models::common::css_enums::TextTransform>) -> Option<String> {
  use lui_models::common::css_enums::TextTransform as Tt;
  match tt {
...
fn font_weight_value(fw: Option<&lui_models::common::css_enums::FontWeight>) -> u16 {
  use lui_models::common::css_enums::FontWeight as Fw;
  match fw {
    Some(Fw::Bold) => 700,
    Some(Fw::Lighter) => 300,
    Some(Fw::Bolder) => 700,
...
fn font_style_axis(fs: Option<&lui_models::common::css_enums::FontStyle>) -> lui_text::FontStyleAxis {
  use lui_models::common::css_enums::FontStyle as Fs;
  use lui_text::FontStyleAxis as A;
  match fs {
