...
use lui_models::{
  Style,
  common::css_enums::{
    CssLength, Cursor, FontStyle, FontWeight, PointerEvents, Resize, TextTransform, UserSelect, WhiteSpace, WordBreak,
  },
};
use lui_text::{FontHandle, FontStyleAxis, ShapedRun};

use crate::{
  Ctx, color::resolve_color, resolved_cursor, resolved_opacity, resolved_user_select, resolved_z_index, types::*,
};
...
fn line_height_px_for_font(style: &Style, font_size: f32, text_ctx: &lui_text::TextContext, handle: FontHandle) -> f32 {
  match style.line_height.as_ref() {
    Some(CssLength::Px(v)) => *v,
    Some(CssLength::Em(v)) | Some(CssLength::Rem(v)) => v * font_size,
    _ => {
