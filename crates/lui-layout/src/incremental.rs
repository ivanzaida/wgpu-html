...
use crate::{
  box_tree::{BoxKind, LayoutBox, LayoutTree, Overflow, ScrollInfo, StickyInsets},
  context::LayoutContext,
  engine::layout_node,
  geometry::{Point, RectEdges},
  text::TextContext,
};
...
impl LayoutCache {
  ...
  pub fn snapshot(tree: &LayoutTree) -> Self {
    ...
    Self {
      boxes,
      old_tree,
      prev_rects_count: tree.rects.len(),
    }
  }
...
impl<'a> CacheView<'a> {
  pub fn try_clone<'b>(
    ...
    match self {
      ...
      CacheView::Incremental { cache, dirty } => try_clone_from_cache(b, cache, dirty, ctx, pos, rects, bump),
    }
  }
}
...
fn expand_dirty_walk(
  ...
  let dominated = dirty_paths
    .iter()
    .any(|dp| path.starts_with(dp) || dp.starts_with(path));
  if !dominated {
    return;
  }
...
fn try_clone_from_cache<'a>(
  ...
  if !cache.is_clean(b.node, dirty) {
    return false;
  }
...
fn synthesize_children<'a>(parent: &mut LayoutBox<'a>, snapshots: &[ChildSnapshot], dx: f32, dy: f32, bump: &'a Bump) {
  parent.children = bumpalo::collections::Vec::from_iter_in(
    snapshots.iter().map(|snap| {
      let node: &'a HtmlNode = unsafe { &*(snap.node_ptr as *const HtmlNode) };
      let style: &'a lui_cascade::ComputedStyle<'a> =
        unsafe { &*(snap.style_ptr as *const lui_cascade::ComputedStyle<'a>) };
...
pub fn layout_incremental_with<'a>(
  ...
  let view = CacheView::Incremental {
    cache: prev_cache,
    dirty: &dirty,
  };
