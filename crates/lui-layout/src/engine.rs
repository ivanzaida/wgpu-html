...
use crate::{
  box_gen::build_box,
  box_tree::{BoxKind, LayoutBox, LayoutTree},
  context::LayoutContext,
  flow,
  geometry::Point,
  incremental::{CacheView, LayoutCache},
  text::TextContext,
};
...
impl LayoutEngine {
  ...
  pub fn layout<'a>(
    ...
    let tree = layout_tree_inner(
      styled,
      viewport_width,
      viewport_height,
      text_ctx,
      self.cache.prev_rects_count,
    );
...
fn apply_text_overflow_ellipsis(b: &mut LayoutBox) {
  ...
  if css_str(b.style.text_overflow) != "ellipsis" {
    return;
  }
  let overflow = css_str(b.style.overflow_x);
  if !matches!(overflow, "hidden" | "clip" | "scroll") {
    return;
  }
