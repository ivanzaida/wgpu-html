...
fn flex_test_html_layout() {
  // Direct port of crates/lui-demo/html/flex-test.html minus the
...
fn img_html_attrs_respected_after_image_loads() {
  ...
  let mut io = AssetIo::new(lui_assets::blocking::BlockingFetcher::new());
...
fn flex_row_percentage_width_plus_flex_grow() {
  ...
  let mut parsed = lui_parser::parse(&html);
  parsed.wrap_body = false;
  parsed.register_system_fonts("sans-serif");
  let cascaded = lui_style::cascade(&parsed);
  let mut text_ctx = lui_text::TextContext::new(64);
  text_ctx.sync_fonts(&parsed.fonts);
  let mut image_cache = AssetIo::new(lui_assets::blocking::BlockingFetcher::new());
