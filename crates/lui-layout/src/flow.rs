...
fn layout_text_node(
    ...
    let transformed: Option<String> = match text_transform {
        ...
        "capitalize" => Some(text.split_whitespace()
            .map(|w| {
                let mut c = w.chars();
                match c.next() {
...
fn layout_inline_container<'a>(
    ...
    let is_anon = matches!(b.kind, BoxKind::AnonymousBlock | BoxKind::AnonymousInline);
    let explicit_w = if is_anon { None } else { sizes::resolve_length(b.style.width, ctx.containing_width) };
    let explicit_h = if is_anon { None } else { sizes::resolve_length(b.style.height, ctx.containing_height) };
