...
//! - `grid-template-columns` / `grid-template-rows` with `<length>` / `<percent>` / `auto` / `<flex>` (`fr`) tracks
//!   plus `repeat(<int>, <list>)` expansion
//! - `grid-auto-rows` / `grid-auto-columns` for implicit tracks
//! - `grid-auto-flow: row | column` (the dense variants are accepted but lay out the same as their non-dense
//!   counterparts for now)
//! - Explicit placement via `grid-column-start/end`, `grid-row-start/end` and the `grid-column` / `grid-row` shorthands
//!   (line numbers and `span <n>`)
//! - Auto-placement of items with no explicit placement, in source order
//! - `gap` / `row-gap` / `column-gap`
//! - `justify-items` / `align-items` (default cell anchoring), `justify-self` / `align-self` (per-item override),
//!   `justify-content` / `align-content` (track block distribution when sized below the container)
//!
...
use lui_models::{
  Style,
  common::css_enums::{
    AlignContent, AlignItems, AlignSelf, CssLength, GridAutoFlow, GridLine, GridTrackSize, JustifyContent,
...
pub(crate) fn layout_grid_children(
  ...
  let auto_flow = parent_style.grid_auto_flow.clone().unwrap_or(GridAutoFlow::Row);
  ...
  let gap_col = length::resolve(
    parent_style.column_gap.as_ref().or(parent_style.gap.as_ref()),
    inner_width,
    ctx,
  )
  .unwrap_or(0.0);
  ...
  let default_align_items = parent_style.align_items.clone().unwrap_or(AlignItems::Stretch);
  let default_justify_items = parent_style.justify_items.clone().unwrap_or(JustifyItems::Stretch);

  // Container distribution along each axis.
  let justify_content = parent_style.justify_content.clone().unwrap_or(JustifyContent::Start);
  let align_content = parent_style.align_content.clone().unwrap_or(AlignContent::Start);
  ...
  // -----------------------------------------------------------------
  let explicit_cols: Vec<GridTrackSize> = parent_style.grid_template_columns.clone().unwrap_or_default();
  let explicit_rows: Vec<GridTrackSize> = parent_style.grid_template_rows.clone().unwrap_or_default();
  let auto_col_size = parent_style.grid_auto_columns.clone().unwrap_or(GridTrackSize::Auto);
  let auto_row_size = parent_style.grid_auto_rows.clone().unwrap_or(GridTrackSize::Auto);
  ...
  // -----------------------------------------------------------------
  let col_widths = resolve_track_sizes(&tracks_x, &items, Axis::Inline, inner_width, gap_col, ctx, parent);
  ...
  let row_heights = resolve_track_sizes(&tracks_y, &items, Axis::Block, block_axis_extent, gap_row, ctx, parent);

  // -----------------------------------------------------------------
  // Track positions (cumulative sums + gaps + axis-content offset).
  // -----------------------------------------------------------------
  let total_col_w = col_widths.iter().sum::<f32>() + gap_col * (col_widths.len() as f32 - 1.0).max(0.0);
  let total_row_h = row_heights.iter().sum::<f32>() + gap_row * (row_heights.len() as f32 - 1.0).max(0.0);
...
/// 2. Process items with both axes definite first — they pin the grid's lower bound on each axis.
/// 3. Process items with only one axis definite, placing them at the next free row / column on the opposite axis.
...
fn place_items(
  ...
  let mut tentative: Vec<TentativePlacement> = items.iter().map(|it| TentativePlacement::from_raw(&it.raw)).collect();
...
fn effective_span(start: &GridLine, end: &GridLine) -> usize {
  match (start, end) {
    (GridLine::Span(n), GridLine::Auto) | (GridLine::Auto, GridLine::Span(n)) => (*n as usize).max(1),
...
fn measure_items(items: &[GridItem], axis: Axis, ctx: &mut Ctx, _parent: &CascadedNode) -> Vec<f32> {
