...
fn inline_text_wraps_at_container_width() {
  ...
  assert!(
    container.content.height > 20.0,
    "text should wrap, height={}",
...
fn inline_short_text_fits_single_line() {
  ...
  assert!(
    container.content.height < 40.0,
    "short text should be one line, height={}",
    container.content.height
  );
}
...
fn inline_block_respects_width_and_height() {
  ...
  fn find_kind<'a>(b: &'a LayoutBox<'a>, kind: BoxKind) -> Option<&'a LayoutBox<'a>> {
    ...
    for c in &b.children {
      if let Some(f) = find_kind(c, kind) {
        return Some(f);
      }
    }
    None
  ...
  let ib = ib.unwrap();
  assert!(
    (ib.content.width - 100.0).abs() < 1.0,
    "width:100px, got {}",
    ib.content.width
  );
  assert!(
    (ib.content.height - 50.0).abs() < 1.0,
    "height:50px, got {}",
    ib.content.height
  );
}
...
fn inline_block_flows_horizontally() {
  ...
  assert!(
    ibs[1].content.x > ibs[0].content.x,
    "second inline-block should be to the right"
  );
}
...
fn inline_block_wraps_when_exceeds_container() {
  ...
  assert_eq!(ibs.len(), 2);
  assert!(
    ibs[1].content.y > ibs[0].content.y,
...
fn inline_block_with_padding_and_border() {
  ...
  assert!(
    (ib.content.width - 80.0).abs() < 1.0,
    "content width should be 80, got {}",
    ib.content.width
  );
}
...
fn white_space_nowrap_prevents_text_wrapping() {
  ...
  assert!(
    anon.content.width > 50.0,
    "white-space:nowrap should prevent wrapping, text width={}",
    anon.content.width
  );
}
...
fn white_space_normal_wraps_text() {
  ...
  let (doc_nowrap, ctx_nowrap) = flex_lt(
    r#"
        <div style="width:50px; white-space:nowrap">Hello World Test</div>
    "#,
    800.0,
  ...
  assert!(
    ...
    div_nowrap.content.height,
    div_wrap.content.height
  );
}
...
fn line_height_inherited_to_anonymous_block() {
  ...
  let anon = &div.children[0];
  assert!(
    anon.content.height >= 59.0,
    "anonymous block should inherit line-height:60px from parent, got height={}",
    anon.content.height
  );
}
...
Line two</div>
  ...
  assert!(
    div.content.height > single_line_h * 1.5,
    "pre-wrap with newline should be taller than single line: got {} vs {}",
    div.content.height,
    single_line_h
  );
}
...
fn word_break_break_all_keeps_width_within_container() {
  ...
  assert!(
    text_box.content.width <= 31.0,
    "word-break:break-all text should fit within 30px container, got {}",
...
fn text_indent_offsets_first_line() {
  ...
  assert!(
    (text_x - 30.0).abs() < 1.0,
    "text-indent:30px should offset text by 30px, got {}",
    text_x
  );
}
...
fn letter_spacing_widens_text() {
  let (doc_normal, ctx_normal) = flex_lt(
    r#"
  ...
  let w_normal = find_by_tag(&lt_normal.root, "body")
    .unwrap()
    .children
    .first()
    .unwrap()
  ...
  let (doc_spaced, ctx_spaced) = flex_lt(
    r#"
  ...
  let w_spaced = find_by_tag(&lt_spaced.root, "body")
    .unwrap()
    .children
    .first()
    .unwrap()
  ...
  assert!(
    w_spaced > w_normal + 15.0,
    "letter-spacing:5px should add ~20px, normal={}, spaced={}",
    w_normal,
    w_spaced
  );
}
...
fn text_transform_uppercase() {
  ...
  let div = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  assert!(
    div.content.height > 0.0,
    "text-transform:uppercase should produce content"
  );
}
...
fn inline_flex_lays_out_children_horizontally() {
  ...
  assert!(
    iflex.children[1].content.x > iflex.children[0].content.x,
...
fn display_contents_promotes_children() {
  ...
  assert_eq!(
    flex.children.len(),
    2,
    "contents wrapper should be transparent, flex should have 2 children, got {}",
    flex.children.len()
  );
}
...
fn display_list_item_has_marker() {
  ...
  fn find_list_item<'a>(b: &'a LayoutBox<'a>) -> Option<&'a LayoutBox<'a>> {
    ...
    for c in &b.children {
      if let Some(f) = find_list_item(c) {
        return Some(f);
      }
    }
    None
...
fn inline_block_shrink_to_fit() {
  ...
  fn find_ib<'a>(b: &'a LayoutBox<'a>) -> Option<&'a LayoutBox<'a>> {
    if b.kind == BoxKind::InlineBlock {
      return Some(b);
    }
    for c in &b.children {
      if let Some(f) = find_ib(c) {
        return Some(f);
      }
    }
    None
...
fn direction_rtl_reverses_inline_order() {
  ...
  if ibs.len() >= 2 {
    ...
    assert!(
      ibs[0].content.x > ibs[1].content.x,
      "rtl: A should be right of B, A.x={} B.x={}",
      ibs[0].content.x,
      ibs[1].content.x
    );
  }
...
fn vertical_align_middle_centers_shorter_child() {
  ...
  if ibs.len() >= 2 {
    ...
    assert!(
      (mid_center - tall_center).abs() < 5.0,
      "vertical-align:middle should center mid_y_center={}, tall_y_center={}",
      mid_center,
      tall_center
    );
  }
...
fn vertical_align_bottom_aligns_shorter_child() {
  ...
  if ibs.len() >= 2 {
    ...
    assert!(
      (bot_bottom - tall_bottom).abs() < 5.0,
      "vertical-align:bottom, bot_bottom={}, tall_bottom={}",
      bot_bottom,
      tall_bottom
    );
  }
