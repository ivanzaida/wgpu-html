...
use lui_layout::{
  LayoutBox,
  engine::{LayoutEngine, layout_tree},
  incremental::layout_tree_incremental,
};
...
fn boxes_match(a: &LayoutBox, b: &LayoutBox) -> bool {
  if (a.content.x - b.content.x).abs() > 0.5 {
    return false;
  }
  if (a.content.y - b.content.y).abs() > 0.5 {
    return false;
  }
  if (a.content.width - b.content.width).abs() > 0.5 {
    return false;
  }
  if (a.content.height - b.content.height).abs() > 0.5 {
    return false;
  }
...
fn incremental_0_dirty_matches_full() {
  ...
  assert!(
    boxes_match(&full.root, &inc.root),
...
fn scroll_state_preserved() {
  ...
  let styled = ctx.cascade(&doc.root, &media, &interaction);
  let full = layout_tree(&styled, 800.0, 600.0);
  let scroll_box = find_by_tag(&full.root, "body").unwrap().children[0].children[0].scroll;
  assert!(scroll_box.is_some(), "should have scroll container");

  let dirty = vec![vec![0, 0, 1]];
  let inc = layout_tree_incremental(&styled, &full, &dirty, 800.0, 600.0);
  let inc_scroll = find_by_tag(&inc.root, "body").unwrap().children[0].children[0].scroll;
...
fn engine_layout_matches_free_function() {
  ...
  assert!(
    boxes_match(&free.root, &eng.root),
...
fn engine_layout_dirty_matches_full() {
  ...
  assert!(
    boxes_match(&full.root, &dirty.root),
