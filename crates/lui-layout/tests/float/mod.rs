...
fn float_left_moves_to_left_edge() {
  ...
  let offset = float_child.content.x
    - float_child.padding.left
    - float_child.border.left
    - float_child.margin.left
    - container.content.x;
  assert!(
    offset.abs() < 1.0,
    "float:left should be at left edge, offset={}",
    offset
  );
}
...
fn float_left_does_not_affect_container_height() {
  ...
  assert!(
    container.content.height < 100.0,
    "container should not extend for float, got {}",
...
fn float_right_moves_to_right_edge() {
  ...
  let container = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  let float_child = &container.children[0];
  let float_right = float_child.content.x
    + float_child.content.width
    + float_child.padding.right
    + float_child.border.right
    + float_child.margin.right;
  let container_right = container.content.x + container.content.width;
  assert!(
    (container_right - float_right).abs() < 1.0,
...
fn clear_left_moves_below_left_float() {
  ...
  let float_bottom = float_child.content.y
    + float_child.content.height
    + float_child.padding.bottom
    + float_child.border.bottom
    + float_child.margin.bottom;
  assert!(
    cleared.content.y >= float_bottom - 1.0,
...
fn clear_both_moves_below_all_floats() {
  ...
  let right_bottom = right_float.content.y
    + right_float.content.height
    + right_float.padding.bottom
    + right_float.border.bottom
    + right_float.margin.bottom;
  assert!(
    cleared.content.y >= right_bottom - 1.0,
...
fn overflow_hidden_contains_floats() {
  ...
  assert!(
    container.content.height >= 199.0,
    "BFC should contain float, height={}",
    container.content.height
  );
}
...
fn multiple_left_floats_stack_horizontally() {
  ...
  assert!(
    b_left >= a_right - 1.0,
    "second left float should be right of first (a_right={}, b_left={})",
    a_right,
    b_left
  );
}
...
fn float_left_and_right_on_same_line() {
  ...
  assert!(
    right.content.x > left.content.x + left.content.width,
...
fn in_flow_block_narrows_beside_float() {
  ...
  let float_right = container.children[0].content.x
    + container.children[0].content.width
    + container.children[0].padding.right
    + container.children[0].border.right
    + container.children[0].margin.right;
  assert!(
    in_flow.content.x >= float_right - 1.0,
