...
fn position_absolute_removed_from_flow() {
    ...
    assert!((container.content.height - 100.0).abs() < 1.0,
        "container height should be ~100 (two 50px in-flow), got {}", container.content.height);
}
...
fn position_relative_offsets_from_normal_position() {
    ...
    assert!((after_offset - 100.0).abs() < 1.0,
        "after should be at normal flow position (100px from before), got {}", after_offset);
    ...
    assert!((shifted_x_offset - 20.0).abs() < 1.0, "left:20px relative offset, got {}", shifted_x_offset);
}
...
fn position_absolute_width_from_left_right() {
    ...
    let container = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
    let abs = &container.children[0];
    assert!((abs.content.width - 300.0).abs() < 1.0,
        "width should be 400-50-50=300, got {}", abs.content.width);
}
...
fn position_absolute_no_insets_uses_static_position() {
    ...
    assert!(abs.content.y >= before.content.y + before.content.height - 1.0,
...
fn position_static_stays_in_flow() {
    ...
    let lt = layout_tree(&styled, 800.0, 600.0);
    let container = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
    assert!((container.content.height - 100.0).abs() < 1.0, "both in flow = 100px height");
}
...
fn position_fixed_uses_viewport() {
    ...
    assert!((fixed.content.x - 10.0).abs() < 1.0, "fixed left:10 from viewport, got {}", fixed.content.x);
    assert!((fixed.content.y - 10.0).abs() < 1.0, "fixed top:10 from viewport, got {}", fixed.content.y);
}
...
fn position_relative_with_right() {
    ...
    assert!((offset - (-20.0)).abs() < 1.0, "right:20 → shift left by 20, offset={}", offset);
}
...
fn position_relative_with_bottom() {
    ...
    assert!((offset - (-15.0)).abs() < 1.0, "bottom:15 → shift up by 15, offset={}", offset);
}
