...
fn em_unit_resolved_at_cascade_time() {
  ...
  assert!(
    ...
    "10em at default 16px should be 160px, got {}",
    el.content.width
  );
}
...
fn rem_unit_resolved_at_cascade_time() {
  ...
  assert!(
    (el.content.width - 160.0).abs() < 1.0,
    "10rem at root 16px should be 160px, got {}",
    el.content.width
  );
}
...
fn em_height_resolved_at_cascade_time() {
  ...
  let el = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  assert!(
    (el.content.height - 48.0).abs() < 1.0,
    "3em at default 16px should be 48px, got {}",
    el.content.height
  );
}
...
fn em_font_size_doubles_parent() {
  ...
  assert!(
    (el.content.width - 160.0).abs() < 1.0,
    "5em at element's 32px font-size should be 160px, got {}",
    el.content.width
  );
}
...
fn calc_resolved_by_cascade() {
  ...
  let el = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  assert!(
    (el.content.width - 150.0).abs() < 1.0,
    "calc(100px + 50px) should be 150px, got {}",
    el.content.width
  );
}
...
fn ch_unit_resolved_by_cascade() {
  ...
  let el = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  assert!(
    (el.content.width - 80.0).abs() < 1.0,
    "10ch at 16px should be ~80px (0.5em approx), got {}",
    el.content.width
  );
}
...
fn ex_unit_resolved_by_cascade() {
  ...
  let el = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  assert!(
    (el.content.height - 32.0).abs() < 1.0,
    "4ex at 16px should be ~32px (0.5em approx), got {}",
    el.content.height
  );
}
...
fn vw_unit_resolves_against_viewport() {
  ...
  let el = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  assert!(
    (el.content.width - 400.0).abs() < 1.0,
    "50vw at 800px viewport should be 400px, got {}",
    el.content.width
  );
}
...
fn vh_unit_resolves_against_viewport() {
  ...
  let el = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
  assert!(
    (el.content.height - 600.0).abs() < 1.0,
    "100vh at 600px viewport should be 600px, got {}",
    el.content.height
  );
}
