...
fn build_box_single_text_node_is_anonymous_inline() {
  ...
  assert_eq!(
    ...
    "text node should produce AnonymousInline box"
  );
}
...
fn build_box_div_without_ua_stylesheet_defaults_to_block() {
  ...
  assert_eq!(
    lb.kind,
    BoxKind::Block,
    "div without UA stylesheet should default to Block"
  );
}
...
fn build_box_inline_text_between_blocks_wrapped_in_anonymous_block() {
  ...
  assert_eq!(
    ...
    "second should be AnonymousBlock wrapping inline text"
  );
  ...
  assert_eq!(
    lb.children[1].children.len(),
    1,
    "anonymous block should contain 1 child"
  );
  assert_eq!(
    lb.children[1].children[0].kind,
    BoxKind::AnonymousInline,
    "anon-block child should be AnonymousInline"
  );
}
...
fn display_none_element_still_in_tree() {
  ...
  assert!(
    container.content.height < 60.0,
    "display:none should not add height, got {}",
