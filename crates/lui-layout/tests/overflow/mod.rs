...
fn overflow_hidden_does_not_change_layout_width() {
    ...
    assert!((container.content.width - 200.0).abs() < 1.0,
        "hidden: no scrollbar reduction, got {}", container.content.width);
}
...
fn overflow_hidden_has_scroll_info() {
    ...
    assert!(scroll.scroll_height >= 299.0,
        "scroll_height should reflect content extent, got {}", scroll.scroll_height);
}
...
fn overflow_scroll_reserves_scrollbar_width() {
    ...
    assert!(container.content.width < 200.0,
        "scroll should reduce content width, got {}", container.content.width);
    assert!((container.content.width - 185.0).abs() < 1.0,
        "200 - 15px scrollbar = 185, got {}", container.content.width);
}
...
fn overflow_scroll_x_reserves_scrollbar_height() {
    ...
    assert!((container.content.height - 85.0).abs() < 1.0,
        "100 - 15px scrollbar = 85, got {}", container.content.height);
}
...
fn overflow_auto_no_scrollbar_when_fits() {
    ...
    assert!((container.content.width - 200.0).abs() < 1.0,
        "auto: no scrollbar when fits, got {}", container.content.width);
}
...
fn overflow_auto_scrollbar_when_overflows() {
    ...
    assert!(container.content.width < 200.0,
        "auto: should reduce width when overflows, got {}", container.content.width);
}
...
fn scrollbar_width_thin() {
    ...
    let container = find_by_tag(&lt.root, "body").unwrap().children.first().unwrap();
    // thin scrollbar = 8px
    assert!((container.content.width - 192.0).abs() < 1.0,
        "thin scrollbar: 200-8=192, got {}", container.content.width);
...
fn scrollbar_width_none() {
    ...
    assert!((container.content.width - 200.0).abs() < 1.0,
        "no scrollbar space with scrollbar-width:none, got {}", container.content.width);
}
...
fn set_scroll_clamps_to_max() {
    ...
    assert!((info.scroll_y - max_y).abs() < 1.0,
        "scroll_y should be clamped to max ({}), got {}", max_y, info.scroll_y);
...
fn child_visible_rect_offsets_by_scroll() {
    ...
    assert!((visible.y - (child_rect.y - 50.0)).abs() < 1.0,
