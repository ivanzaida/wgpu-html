//! Tree-shape parse tests: nesting, attributes, text mixing, auto-close,
//! synthetic body wrapping, and inline `style` parsing.

use wgpu_html_parser::{parse, parse_inline_style};
...
fn input_type_value() {
  ...
  // Type parses to InputType::Email
...
fn form_with_inputs_and_button() {
  ...
  let kinds: Vec<_> = r.children.iter().map(|c| std::mem::discriminant(&c.element)).collect();
...
fn p_auto_closes_when_div_opens() {
  // <p>...<div>... → </p> implied before <div>
  let r = root("<p>a<div>b</div>");
  // Two top-level → synthetic body wrapper
  assert!(matches!(r.element, Element::Body(_)));
  assert_eq!(r.children.len(), 2);
  assert!(matches!(r.children[0].element, Element::P(_)));
...
fn inline_style_color_padding() {
  ...
  // The CSS parser fills concrete fields; checking presence is enough here.
