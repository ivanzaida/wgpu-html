...
fn parse_media_feature(input: &str) -> Option<MediaFeature> {
    ...
    match name.as_str() {
        ...
        "orientation" if value.eq_ignore_ascii_case("portrait") => {
            Some(MediaFeature::OrientationPortrait)
        }
        "orientation" if value.eq_ignore_ascii_case("landscape") => {
            Some(MediaFeature::OrientationLandscape)
        }
        _ => None,
    }
}
...
fn parse_rules(input: &str, media_stack: &mut Vec<MediaQueryList>, rules: &mut Vec<Rule>) {
    ...
    while cursor < input.len() {
        ...
        if let Some(query) = strip_ascii_word_prefix(header, "@media").and_then(|h| {
            ...
            (!h.is_empty())
                .then_some(h)
                .and_then(parse_media_query_list)
        }) {
...
mod tests {
    ...
    fn parses_not_media_modifier() {
        ...
        assert_eq!(
            list.queries[0].features,
            vec![MediaFeature::MinWidth(300.0)]
        );
    }
