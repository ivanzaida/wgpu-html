...
//!
//! 1. **Type translators / forwarders** (the bulk of this file). Free functions that turn winit's `KeyEvent` /
//!    `MouseButton` / `KeyCode` into the engine's [`MouseButton`], [`Modifier`], and DOM-style key-name strings, and
//!    feed them into the dispatch API on [`Tree`]. Apps that already have their own event loop wire these in by hand.
//!
//! 2. **A batteries-included harness** ([`WgpuHtmlWindow`]) that owns the window, renderer, text context, and the full
...
pub use window::{AppHook, EventResponse, FrameTimings, HookContext, HtmlWindow, WgpuHtmlWindow, create_window};
use winit::{
