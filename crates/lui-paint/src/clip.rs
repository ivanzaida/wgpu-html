...
pub fn should_clip(b: &LayoutBox) -> bool {
    matches!(b.overflow_x, Overflow::Hidden | Overflow::Scroll | Overflow::Auto | Overflow::Clip)
        || matches!(b.overflow_y, Overflow::Hidden | Overflow::Scroll | Overflow::Auto | Overflow::Clip)
}
...
pub fn push_overflow_clip(
    ...
    let frame = ClipFrame { rect: clipped, radii_h: inner_h, radii_v: inner_v };
    clip_stack.push(frame.clone());
    dl.push_clip(clipped, inner_h, inner_v);
    parent
}

pub fn pop_overflow_clip(
    parent: &ClipFrame,
    clip_stack: &mut Vec<ClipFrame>,
    dl: &mut DisplayList,
) {
