...
pub(crate) fn compute_value_run(
  ...
  let (value, is_password, wraps_multiline) = match &node.element {
    Element::Input(inp) => {
      if matches!(
        inp.r#type,
        Some(InputType::Hidden | InputType::Checkbox | InputType::Radio | InputType::Range | InputType::Color)
      ) {
        return (None, None);
      }
      ...
      if matches!(inp.r#type, Some(InputType::Date) | Some(InputType::DatetimeLocal)) {
        let is_focused = ctx.date_display_value.is_some() && ctx.date_focus_iso.as_deref() == inp.value.as_deref();
  ...
  if !wraps_multiline {
    if let Some(run) = run.as_mut() {
      ...
      if matches!(node.style.text_align, Some(css_enums::TextAlign::Center)) {
...
pub(crate) fn compute_placeholder_run(
  ...
  if !wraps_multiline {
    if let Some(run) = run.as_mut() {
      ...
      // 2. Vertical centering — centre within the padding box so text appears centered in the full input, not just the
      //    content area.
      vcenter_run_in_rect(run, content_rect.h);
    }
  }
