...
//! - `flex-grow` / `flex-shrink` / `flex-basis` (with min/max clamping and the canonical iterative freeze loop)
//! - `flex` shorthand (already expanded by the parser into the three longhands)
//! - `justify-content` (flex-start / flex-end / center / space-between / space-around / space-evenly, plus the
//!   start/end/left/right aliases)
...
//! - `min-width` / `max-width` / `min-height` / `max-height` clamping on flex items (via the shared `clamp_axis` helper
//!   in `lib.rs`)
//! - `margin: auto` on flex items: absorbs free space on the main axis (split across all auto sides) and consumes
//!   leftover line cross space on the cross axis
//! - Baseline alignment falls back to `flex-start` for now (we can honour first-line baselines once a richer
//!   ascent/descent path is wired through block layout — only the inline pass currently tracks them).
...
use crate::{
  BlockOverrides, Ctx, LayoutBox, is_auto_margin, layout_block_at_with, length, measure_text_leaf,
...
pub(crate) fn layout_flex_children(
  ...
  let direction = parent_style.flex_direction.clone().unwrap_or(FlexDirection::Row);
  ...
  let align_items = parent_style.align_items.clone().unwrap_or(AlignItems::Stretch);
  let align_content = parent_style.align_content.clone().unwrap_or(AlignContent::Normal);

  let is_row = matches!(direction, FlexDirection::Row | FlexDirection::RowReverse);
  let is_dir_reverse = matches!(direction, FlexDirection::RowReverse | FlexDirection::ColumnReverse);
  ...
  for (line_idx, line) in lines.iter().enumerate() {
    ...
    for &i in line {
      ...
      let auto_pre = if item.auto_main_starts() { auto_main_each } else { 0.0 };
      let auto_post = if item.auto_main_ends() { auto_main_each } else { 0.0 };
      ...
      let align = resolve_align_self(&item.align_self, &align_items);
      let stretched =
        matches!(align, ResolvedAlign::Stretch) && !item.has_explicit_cross_size && !auto_cross_pre && !auto_cross_post;
  ...
  if is_dir_reverse && main_axis_size.is_some_and(|m| m > 0.0) {
    ...
    for (_, b) in final_boxes.iter_mut() {
      let main_size_box = if is_row { b.margin_rect.w } else { b.margin_rect.h };
      if is_row {
...
fn build_item<'a>(
  ...
  let mut bw = |width: Option<&CssLength>, bstyle: &Option<lui_models::common::css_enums::BorderStyle>| -> f32 {
    if native {
      return 0.0;
    }
    if matches!(
  ...
  // - `flex-basis: auto` (default) → fall back to the main-axis size property (`width` for row, `height` for column).
  // - Neither set → the item's intrinsic content size. For replaced elements (`<img>`) we use the image's HTML
  //   width/height attributes, falling back to its decoded dimensions; for non-replaced items the approximation is
  //   still 0.
...
fn side_pad(specific: &Option<CssLength>, shorthand: &Option<CssLength>, container: Option<f32>, ctx: &mut Ctx) -> f32 {
  resolve_axis_length(specific.as_ref(), container, ctx)
    .or_else(|| resolve_axis_length(shorthand.as_ref(), container, ctx))
    .unwrap_or(0.0)
}
...
fn resolve_flexible_lengths(items: &mut [FlexItem<'_>], line: &[usize], main_axis_size: f32, gap_main: f32) {
  ...
  let initial_outer_total: f32 = line.iter().map(|&i| items[i].hypothetical_outer_main()).sum::<f32>()
