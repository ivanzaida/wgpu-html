...
fn matches_direct_child_as_descendant() {
    ...
    let ancestors = [AncestorEntry { node: outer, ctx: child_ctx(0, 1) }];
    assert!(matches_selector(&sel.0[0], span, &child_ctx(0, 1), &ancestors, Some(outer)));
}
...
fn matches_grandchild_as_descendant() {
    ...
    let sel = parse_selector_list(".outer span").unwrap();
    let ancestors = [
        AncestorEntry { node: p, ctx: child_ctx(0, 1) },
        AncestorEntry { node: outer, ctx: child_ctx(0, 1) },
    ];
...
fn rejects_when_ancestor_absent() {
    ...
    let ancestors = [AncestorEntry { node: div, ctx: child_ctx(0, 1) }];
    assert!(!matches_selector(&sel.0[0], span, &child_ctx(0, 1), &ancestors, Some(div)));
}
...
fn multi_level_descendant() {
    ...
    let ancestors = [
        AncestorEntry { node: p, ctx: child_ctx(0, 1) },
        AncestorEntry { node: div, ctx: child_ctx(0, 1) },
