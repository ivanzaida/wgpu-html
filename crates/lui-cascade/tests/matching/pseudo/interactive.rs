...
fn hover_matches_when_hovered() {
    ...
    let ctx = MatchContext { is_hover: true, ..root_ctx() };
...
fn active_matches_when_active() {
    ...
    let ctx = MatchContext { is_active: true, ..root_ctx() };
...
fn focus_matches_when_focused() {
    ...
    let ctx = MatchContext { is_focus: true, ..root_ctx() };
...
fn focus_visible_matches() {
    ...
    let ctx = MatchContext { is_focus_visible: true, ..root_ctx() };
    assert!(any_selector_matches(&sel, inp, &ctx, &[], None).is_some());
}
