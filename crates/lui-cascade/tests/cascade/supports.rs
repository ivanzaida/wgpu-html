...
fn supports_unknown_property_skipped() {
  assert!(!cascade_check(
    "<div></div>",
    "@supports (foo-bar: baz) { div { color: red; } }"
  ));
}
...
fn supports_not_unknown_applies() {
  assert!(cascade_check(
    "<div></div>",
    "@supports not (foo-bar: baz) { div { color: red; } }"
  ));
}
...
fn supports_and_one_unknown_skipped() {
  assert!(!cascade_check(
    "<div></div>",
    "@supports (display: grid) and (fake-prop: x) { div { color: red; } }"
  ));
}
...
fn supports_or_mixed_applies() {
  assert!(cascade_check(
    "<div></div>",
    "@supports (fake-prop: x) or (display: block) { div { color: red; } }"
  ));
}
...
fn supports_or_all_unknown_skipped() {
  assert!(!cascade_check(
    "<div></div>",
    "@supports (fake-prop: x) or (bogus: y) { div { color: red; } }"
  ));
}
...
fn normal_and_supports_rules_combine() {
  ...
  let mut ctx = CascadeContext::new();
  ctx.set_stylesheets(&[
    parse_stylesheet("div { display: block; } @supports (display: grid) { div { color: red; } }").unwrap(),
  ]);
  let media = MediaContext::default();
  let interaction = InteractionState::default();
  let styled = ctx.cascade(&doc.root, &media, &interaction);
...
fn normal_outside_and_failed_supports_inside() {
  ...
  let mut ctx = CascadeContext::new();
  ctx.set_stylesheets(&[
    parse_stylesheet("div { display: block; } @supports (fake: x) { div { color: red; } }").unwrap(),
  ]);
  let media = MediaContext::default();
  let interaction = InteractionState::default();
  let styled = ctx.cascade(&doc.root, &media, &interaction);
