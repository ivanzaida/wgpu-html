...
fn cache_distinguishes_different_ids() {
  ...
  let mut ctx = CascadeContext::new();
  ctx.set_stylesheets(&[parse_stylesheet("#x { color: red; } #y { color: blue; }").unwrap()]);
  let media = MediaContext::default();
  let interaction = InteractionState::default();
  let styled = ctx.cascade(&doc.root, &media, &interaction);
...
fn cache_distinguishes_hover_state() {
  ...
  let mut ctx = CascadeContext::new();
  ctx.set_stylesheets(&[parse_stylesheet(".btn { color: red; } .btn:hover { color: blue; }").unwrap()]);
  let media = MediaContext::default();
...
fn many_identical_list_items_all_styled() {
  ...
  let doc = parse(&html);
  let mut ctx = CascadeContext::new();
  ctx.set_stylesheets(&[parse_stylesheet(".item { color: red; padding: 4px; margin: 2px; display: block; }").unwrap()]);
  let media = MediaContext::default();
  let interaction = InteractionState::default();
  let styled = ctx.cascade(&doc.root, &media, &interaction);
...
fn cache_distinguishes_ancestor_context() {
  ...
  let mut ctx = CascadeContext::new();
  ctx.set_stylesheets(&[parse_stylesheet(".container .item { color: red; } .sidebar .item { color: blue; }").unwrap()]);
  let media = MediaContext::default();
  let interaction = InteractionState::default();
  let styled = ctx.cascade(&doc.root, &media, &interaction);
  ...
  assert_ne!(
    item1.style.color.unwrap(),
    item2.style.color.unwrap(),
...
fn cache_distinguishes_attribute_selectors() {
  ...
  let mut ctx = CascadeContext::new();
  ctx.set_stylesheets(&[parse_stylesheet(r#"[type="text"] { color: red; } [type="email"] { color: blue; }"#).unwrap()]);
  let media = MediaContext::default();
  let interaction = InteractionState::default();
  let styled = ctx.cascade(&doc.root, &media, &interaction);
  ...
  assert!(
    stats.hits >= 1,
    "identical middle inputs should cache-hit, got {} hits",
    stats.hits
  );
}
...
fn identical_items_in_same_ancestor_still_cache() {
  ...
  assert!(
    stats.hits >= 2,
    "identical items in same ancestor should cache-hit, got {} hits",
    stats.hits
  );
}
...
fn stats_zero_hits_when_all_unique() {
  ...
  assert_eq!(
    stats.hits, 0,
    "expected 0 hits for all-unique elements, got {}",
    stats.hits
  );
}
