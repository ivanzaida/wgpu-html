...
fn var_chain_across_inheritance() {
    ...
    grandparent.custom_properties
        .get_or_insert_with(Default::default)
        .insert(ArcStr::from("--base"), base);

    // Parent defines --gap: var(--base), inherits --base
    let gap_var = alloc_value(&arena, CssValue::Var {
    ...
    let row_gap_var = alloc_value(&arena, CssValue::Var {
...
fn multiple_vars_from_same_inherited_source() {
    ...
    let color_var = alloc_value(&arena, CssValue::Var {
        name: ArcStr::from("--theme"),
        fallback: None,
    });
    let bg_var = alloc_value(&arena, CssValue::Var {
        name: ArcStr::from("--theme"),
        fallback: None,
    });
