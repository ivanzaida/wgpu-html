    ...
    while compound_idx > 0 {
        ...
        match combinator {
            ...
            CssCombinator::SubsequentSibling => {
                ...
                let found = siblings.iter().any(|(sib_node, sib_ctx, sib_parent)| {
                    matches_compound(compound, sib_node, sib_ctx, *sib_parent, &[])
                });
                if !found {
                    return false;
                }
    ...
    match &pseudo.pseudo {
        ...
        CssPseudo::FirstOfType => match parent_node {
            Some(parent) => {
                ...
                !parent.children[..ctx.sibling_index].iter().any(|c| c.element.tag_name() == tag)
            }
            None => true,
        },
        CssPseudo::LastOfType => match parent_node {
            Some(parent) => {
                let tag = node.element.tag_name();
                !parent.children[ctx.sibling_index + 1..].iter().any(|c| c.element.tag_name() == tag)
            }
            None => true,
        },
        ...
        CssPseudo::NthOfType => match parent_node {
            Some(parent) => {
                let tag = node.element.tag_name();
                let type_index = parent.children[..ctx.sibling_index].iter()
                    .filter(|c| c.element.tag_name() == tag).count();
        ...
        CssPseudo::NthLastOfType => match parent_node {
            Some(parent) => {
                let tag = node.element.tag_name();
                let from_end = parent.children[ctx.sibling_index + 1..].iter()
                    .filter(|c| c.element.tag_name() == tag).count();
                match_nth(pseudo.arg.as_deref(), from_end)
            }
            None => match_nth(pseudo.arg.as_deref(), 0),
        ...
        CssPseudo::Link | CssPseudo::AnyLink => {
            matches!(node.element.tag_name(), "a" | "area" | "link")
                && node.attrs.contains_key("href")
        }
        ...
        CssPseudo::PlaceholderShown => {
            node.attrs.contains_key("placeholder") && node.children.is_empty()
        }
        CssPseudo::Default => node.attrs.contains_key("default"),
        CssPseudo::Valid | CssPseudo::Invalid |
        CssPseudo::UserValid | CssPseudo::UserInvalid |
        CssPseudo::InRange | CssPseudo::OutOfRange => false,
        ...
        CssPseudo::PopoverOpen => node.attrs.contains_key("popover")
        ...
        CssPseudo::Playing | CssPseudo::Paused | CssPseudo::Seeking |
        CssPseudo::Buffering | CssPseudo::Muted | CssPseudo::Stalled |
        ...
        CssPseudo::Before | CssPseudo::After | CssPseudo::FirstLine |
        CssPseudo::FirstLetter | CssPseudo::Placeholder | CssPseudo::Selection |
        CssPseudo::Marker | CssPseudo::Backdrop |
        CssPseudo::BeforeLegacy | CssPseudo::AfterLegacy |
        CssPseudo::FirstLineLegacy | CssPseudo::FirstLetterLegacy |
        CssPseudo::Part | CssPseudo::Slotted |
        CssPseudo::Highlight | CssPseudo::GrammarError | CssPseudo::SpellingError |
        CssPseudo::TargetText | CssPseudo::SearchText |
        CssPseudo::ViewTransition | CssPseudo::ViewTransitionGroup |
        CssPseudo::ViewTransitionGroupChildren |
        CssPseudo::ViewTransitionImagePair |
        CssPseudo::ViewTransitionNew | CssPseudo::ViewTransitionOld |
        CssPseudo::CueFn | CssPseudo::CueRegionFn |
        CssPseudo::NthFragment |
        CssPseudo::Column |
        CssPseudo::FileSelectorButton |
        CssPseudo::Picker | CssPseudo::PickerIcon |
        CssPseudo::Checkmark | CssPseudo::ClearIcon | CssPseudo::ColorSwatch |
        CssPseudo::RevealIcon | CssPseudo::ScrollButton |
        CssPseudo::ScrollMarker | CssPseudo::ScrollMarkerGroup |
        CssPseudo::SliderFill | CssPseudo::SliderThumb | CssPseudo::SliderTrack |
        CssPseudo::StepControl | CssPseudo::StepDown | CssPseudo::StepUp |
        CssPseudo::FieldComponent | CssPseudo::FieldSeparator | CssPseudo::FieldText |
        CssPseudo::DetailsContent => true,
        ...
        match lui_parse::parse_selector_list(rest.trim()) {
            Ok(inner) => node.children.iter().enumerate().any(|(i, child)| {
                ...
                inner.0.iter().any(|sel| {
                    matches_compound_simple(&sel.compounds, child, &ctx, Some(node))
                })
            }),
            Err(_) => false,
        }
...
fn has_matching_descendant(root: &HtmlNode, selectors: &SelectorList) -> bool {
    for (i, child) in root.children.iter().enumerate() {
        ...
        if selectors.0.iter().any(|sel| {
            matches_selector_for_has(sel, child, &ctx, root)
        }) {
            return true;
        }
