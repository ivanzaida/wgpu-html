...
pub fn matches(node: &HtmlNode, selector: &str) -> bool {
    ...
    sel.0.iter().any(|complex| {
        matches_selector(complex, node, &ctx, &[], None)
    })
}
...
pub fn closest<'a>(
    ...
    let ctx = MatchContext::default();
    if sel.0.iter().any(|complex| {
        matches_selector(complex, node, &ctx, ancestors, parent_node)
    }) {
        return Some(node);
    }
    ...
    for (i, entry) in ancestors.iter().enumerate() {
        ...
        if sel.0.iter().any(|complex| {
            matches_selector(complex, entry.node, &entry.ctx, further, parent)
        }) {
...
fn walk_collect<'a>(
    ...
    for (i, child) in node.children.iter().enumerate() {
        ...
        child_ancestors.extend(ancestors.iter().map(|a| AncestorEntry { node: a.node, ctx: a.ctx.clone() }));

        if selectors.iter().any(|sel| {
            matches_selector(sel, child, &ctx, &child_ancestors, Some(node))
        }) {
            results.push(child);
        }
