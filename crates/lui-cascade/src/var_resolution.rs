...
fn resolve_custom_property_chains<'a>(style: &ComputedStyle<'a>, arena: &'a Bump) -> FxHashMap<ArcStr, &'a CssValue> {
...
fn resolve_property_vars<'a>(
  ...
  resolve_field! {
      ...
      overflow_x, overflow_y, scrollbar_color, scrollbar_gutter, scrollbar_width,
  ...
  if let Some(extra) = &mut style.extra {
    let needs_resolve: Vec<CssProperty> = extra
      .iter()
