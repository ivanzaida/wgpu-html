...
fn build_stylesheet(num_rules: usize) -> String {
  ...
  for i in 0..num_rules {
    css.push_str(&format!(
      ".c{} {{ color: red; padding: {}px; margin: {}px; }}\n",
      i % 10,
      i,
      i * 2
    ));
...
fn bench_incremental_cascade(c: &mut Criterion) {
  ...
  group.bench_function("full_plus_clone_0_dirty", |b| {
    ...
    b.iter(|| {
      ...
      let styled = ctx.cascade_dirty(&doc.root, &prev, &[], &media, &interaction);
      black_box(&styled);
    });
  });
  ...
  group.bench_function("full_plus_dirty_1_leaf", |b| {
    ...
    b.iter(|| {
      let prev = ctx.cascade(&doc.root, &media, &interaction);
      let styled = ctx.cascade_dirty(&doc.root, &prev, &[vec![0, 0, 0, 0, 0]], &media, &interaction);
      black_box(&styled);
    });
  });
  ...
  group.bench_function("full_plus_dirty_1_subtree", |b| {
    ...
    b.iter(|| {
      let prev = ctx.cascade(&doc.root, &media, &interaction);
      let styled = ctx.cascade_dirty(&doc.root, &prev, &[vec![0, 0]], &media, &interaction);
      black_box(&styled);
    });
  });
