...
use crate::{
  ResolverContext,
  vars::{resolve_custom_properties, resolve_var_value},
};
...
impl ResolutionContext {
  ...
  pub fn register(&mut self, name: &str, handler: impl Fn(&[CssValue], &ResolverContext, &Bump) -> CssValue + 'static) {
    ...
    match value {
      ...
      CssValue::Function { function, args } => {
        ...
        let result = match function {
          ...
          CssFunction::Round => match args.last() {
            Some(CssValue::Dimension { value, unit }) => CssValue::Dimension {
              value: value.round(),
              unit: unit.clone(),
            },
          ...
          CssFunction::Progress => {
            if args.len() < 3 {
              return arena.alloc(CssValue::Number(0.0));
            }
      ...
      CssValue::Dimension { value: dim_val, unit } => {
        ...
        if let Some(px) = px {
          if px != v || *unit != CssUnit::Px {
            return arena.alloc(CssValue::Dimension {
              value: px,
              unit: CssUnit::Px,
            });
          }
  ...
  fn resolve_var<'a>(&self, value: &'a CssValue, arena: &'a Bump) -> &'a CssValue {
    let raw: FxHashMap<ArcStr, &CssValue> = self.custom_properties.iter().map(|(k, v)| (k.clone(), v)).collect();
...
fn evaluate_min_max(args: &[CssValue], env: &ResolverContext, is_min: bool) -> CssValue {
  ...
  let best_idx = if is_min {
    numbers
      .iter()
      .enumerate()
      .min_by(|(_, a), (_, b)| ResolvedNumber::partial_cmp_rn(a, b))
  } else {
    numbers
      .iter()
      .enumerate()
      .max_by(|(_, a), (_, b)| ResolvedNumber::partial_cmp_rn(a, b))
  };
  match best_idx {
    Some((idx, _)) => numbers[idx].to_css_value(),
    None => CssValue::Number(0.0),
  }
}
...
fn evaluate_clamp(args: &[CssValue], env: &ResolverContext) -> CssValue {
  ...
  if val.as_f64() <= min.as_f64() {
    return args[0].clone();
  }
  if val.as_f64() >= max.as_f64() {
    return args[2].clone();
  }
  args[1].clone()
}
