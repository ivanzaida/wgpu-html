...
use crate::{
  ResolverContext,
  math::resolve_math,
  math_helpers::{ResolvedNumber, to_f64},
};
...
pub fn resolve_units(value: &CssValue, ctx: &ResolverContext) -> CssValue {
  match value {
    CssValue::Number(_) | CssValue::String(_) | CssValue::Color(_) | CssValue::Url(_) | CssValue::Var { .. } => {
      value.clone()
    }
...
pub fn resolve_dimension(value: f64, unit: &CssUnit, ctx: &ResolverContext) -> CssValue {
  match resolve_to_px(value, unit, ctx) {
    Some(px) => CssValue::Dimension {
      value: px,
      unit: CssUnit::Px,
    },
    None => CssValue::Dimension {
      value,
      unit: unit.clone(),
    },
  }
...
fn resolve_calc_value_units(value: &CssValue, ctx: &ResolverContext) -> ResolvedNumber {
  match value {
    ...
    CssValue::Dimension { value, unit } => match resolve_to_px(*value, unit, ctx) {
...
fn resolve_min_max_units(args: &[CssValue], ctx: &ResolverContext, is_min: bool) -> CssValue {
  ...
  match best_idx {
    Some((idx, _)) => resolved.swap_remove(idx),
