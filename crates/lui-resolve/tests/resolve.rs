...
use lui_resolve::{ResolutionContext, ResolverContext, resolve};
...
fn abs_negative_returns_positive() {
  assert_eq!(
    resolve_fn(CssFunction::Abs, vec![CssValue::Number(-5.0)]),
    CssValue::Number(5.0)
  );
}
...
fn abs_positive_returns_same() {
  assert_eq!(
    resolve_fn(CssFunction::Abs, vec![CssValue::Number(3.14)]),
    CssValue::Number(3.14)
  );
}
...
fn abs_zero() {
  assert_eq!(
    resolve_fn(CssFunction::Abs, vec![CssValue::Number(0.0)]),
    CssValue::Number(0.0)
  );
}
...
fn sign_negative() {
  assert_eq!(
    resolve_fn(CssFunction::Sign, vec![CssValue::Number(-5.0)]),
    CssValue::Number(-1.0)
  );
}
...
fn sign_positive() {
  assert_eq!(
    resolve_fn(CssFunction::Sign, vec![CssValue::Number(3.14)]),
    CssValue::Number(1.0)
  );
}
...
fn sign_zero() {
  ...
  assert_eq!(
    resolve_fn(CssFunction::Sign, vec![CssValue::Number(0.0)]),
    CssValue::Number(1.0)
  );
}
...
fn sqrt_perfect_square() {
  assert_eq!(
    resolve_fn(CssFunction::Sqrt, vec![CssValue::Number(16.0)]),
    CssValue::Number(4.0)
  );
}
...
fn sqrt_zero() {
  assert_eq!(
    resolve_fn(CssFunction::Sqrt, vec![CssValue::Number(0.0)]),
    CssValue::Number(0.0)
  );
}
...
fn exp_zero_returns_one() {
  assert_eq!(
    resolve_fn(CssFunction::Exp, vec![CssValue::Number(0.0)]),
    CssValue::Number(1.0)
  );
}
...
fn pow_two_cubed() {
  assert_eq!(
    resolve_fn(CssFunction::Pow, vec![CssValue::Number(2.0), CssValue::Number(3.0)]),
    CssValue::Number(8.0)
  );
}
...
fn pow_four_to_half_returns_two() {
  assert_eq!(
    resolve_fn(CssFunction::Pow, vec![CssValue::Number(4.0), CssValue::Number(0.5)]),
    CssValue::Number(2.0)
  );
}
...
fn pow_zero_exp_returns_one() {
  assert_eq!(
    resolve_fn(CssFunction::Pow, vec![CssValue::Number(42.0), CssValue::Number(0.0)]),
    CssValue::Number(1.0)
  );
}
...
fn modulo_ten_three() {
  assert_eq!(
    resolve_fn(CssFunction::Mod, vec![CssValue::Number(10.0), CssValue::Number(3.0)]),
    CssValue::Number(1.0)
  );
}
...
fn rem_ten_three() {
  assert_eq!(
    resolve_fn(CssFunction::Rem, vec![CssValue::Number(10.0), CssValue::Number(3.0)]),
    CssValue::Number(1.0)
  );
}
...
fn hypot_three_four_returns_five() {
  assert_eq!(
    resolve_fn(CssFunction::Hypot, vec![CssValue::Number(3.0), CssValue::Number(4.0)]),
    CssValue::Number(5.0)
  );
}
...
fn hypot_single_arg() {
  assert_eq!(
    resolve_fn(CssFunction::Hypot, vec![CssValue::Number(5.0)]),
    CssValue::Number(5.0)
  );
}
...
fn round_up() {
  assert_eq!(
    resolve_fn(CssFunction::Round, vec![CssValue::Number(3.6)]),
    CssValue::Number(4.0)
  );
}
...
fn round_down() {
  assert_eq!(
    resolve_fn(CssFunction::Round, vec![CssValue::Number(3.4)]),
    CssValue::Number(3.0)
  );
}
...
fn round_exact_integer() {
  assert_eq!(
    resolve_fn(CssFunction::Round, vec![CssValue::Number(10.0)]),
...
fn progress_midpoint() {
  assert_eq!(
    resolve_fn(
      ...
      vec![CssValue::Number(0.0), CssValue::Number(100.0), CssValue::Number(50.0),]
    ),
    CssValue::Number(0.5),
  );
}
...
fn progress_below_start_clamped_to_zero() {
  assert_eq!(
    resolve_fn(
      CssFunction::Progress,
      vec![CssValue::Number(0.0), CssValue::Number(100.0), CssValue::Number(-10.0),]
    ),
    CssValue::Number(0.0),
  );
}
...
fn progress_above_end_clamped_to_one() {
  assert_eq!(
    resolve_fn(
      CssFunction::Progress,
      vec![CssValue::Number(0.0), CssValue::Number(100.0), CssValue::Number(200.0),]
    ),
    CssValue::Number(1.0),
  );
}
...
fn progress_start_equal_end_returns_zero() {
  ...
  let v = CssValue::Function {
    ...
    args: vec![CssValue::Number(50.0), CssValue::Number(50.0), CssValue::Number(50.0)],
  };
...
fn calc_cannot_be_overridden_by_custom() {
  let mut rctx = ResolutionContext::new(ResolverContext::default());
  rctx.register("calc", |_args, _env, _arena| CssValue::Number(999.0));
  let arena = Bump::new();
  let v = CssValue::Function {
    function: CssFunction::Calc,
    args: vec![
...
fn non_numeric_arg_in_math2_defaults_to_zero() {
  ...
  assert_eq!(
    resolve_fn(
      CssFunction::Pow,
      vec![CssValue::String("x".into()), CssValue::Number(3.0),]
    ),
...
fn hypot_with_percentages() {
  let r = resolve_fn(
    CssFunction::Hypot,
    vec![CssValue::Percentage(3.0), CssValue::Percentage(4.0)],
  );
...
fn log_with_three_args_still_uses_first_two() {
  ...
  let r = resolve_fn(
    CssFunction::Log,
    vec![CssValue::Number(100.0), CssValue::Number(10.0), CssValue::Number(999.0)],
  );
  assert!(approx_eq(as_num(&r), 2.0));
}
