...
//!      `:only-child`, `:empty`, `:root`, `:scope`,
//!      `:nth-child`, `:nth-last-child`, `:first-of-type`,
//!      `:last-of-type`, `:nth-of-type`
//!   3. State pseudo-classes — `:disabled`, `:enabled`,
//!      `:checked`, `:required`, `:optional`,
//!      `:read-only`, `:read-write`, `:placeholder-shown`
//!   4. Interaction pseudo-classes — `:hover`, `:focus`,
//!      `:active`, `:focus-within`
//!   5. `:lang(...)`, `:dir(...)`
//!   6. Pseudo-elements — `::before`, `::after`, `::first-line`
//!      (parser must accept; selector matches nothing)
...
use wgpu_html_tree::{Node, SelectorList, Tree};
...
fn checked_matches_option_with_selected() {
    ...
    let body = Node::new(m::Body::default()).with_children(vec![
        Node::new(m::Select::default()).with_children(vec![
...
fn lang_inherits_from_ancestor() {
    ...
    let body = Node::new(m::Body::default()).with_children(vec![
        Node::new(outer).with_children(vec![Node::new(inner_div)]),
    ]);
    let mut tree = Tree::new(body);
...
fn path_returned_matches_path_query_selector_dereferences() {
    ...
    let id_via_borrow: Option<String> = tree
