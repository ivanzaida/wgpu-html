...
fn pointer_events_none_makes_element_transparent_to_click() {
  ...
  assert!(!overlay_clicked.load(Ordering::Relaxed), "overlay with pointer-events:none should not receive click");
  assert!(behind_clicked.load(Ordering::Relaxed), "element behind should receive click through overlay");
}
