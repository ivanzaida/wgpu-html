...
use lui_tree::{self, Element, FontFace, FontStyleAxis};
...
fn each_input_type_gets_its_own_value_via_text_input() {
  ...
  for &(type_name, expect_editable) in cases {
    ...
    // Try typing "X".
    let mutated = lui_tree::text_input(&mut tree, "X");
    ...
    if expect_editable {
      ...
      if is_date {
        ...
        let mutated2 = lui_tree::text_input(&mut tree, "5");
        assert!(
          mutated2,
          "type={type_name}: text_input('5') should have mutated the display value"
        );
        let dv = tree.interaction.date_display_value.as_deref().unwrap_or("");
        assert!(
          dv.contains('5'),
          "type={type_name}: date_display_value should contain '5', got {dv:?}"
        );
      } else {
...
/// 5. A neighbouring `<input type="text">` with the same typed text stores and displays its value in cleartext.
#[test]
fn password_input_stores_cleartext_but_renders_bullets() {
  ...
  for ch in typed.chars() {
    lui_tree::text_input(&mut tree, &ch.to_string());
  }
  ...
  assert_eq!(txt_run.text, typed, "text input should shape the actual value");
}
