...
/// Winit + wgpu driver. Stores config until `run()` creates the real window.
...
impl WinitDriver {
    pub fn new(width: u32, height: u32, title: &str) -> Self {
        Self { width, height, title: title.to_string() }
    }
}
    ...
    fn run(self: Box<Self>, mut lui: Lui) {
        ...
        impl ApplicationHandler for App {
            fn resumed(&mut self, event_loop: &ActiveEventLoop) {
                ...
                self.lui.renderer = Some(Box::new(pollster::block_on(
                ...
                match &event {
                    ...
                    WindowEvent::RedrawRequested => {
                        ...
                        if matches!(outcome, crate::display_list::FrameOutcome::Reconfigure) {
                            if let Some(r) = &mut self.lui.renderer { r.resize(size.width, size.height); }
                    ...
                    WindowEvent::ScaleFactorChanged { .. } => {
                        let s = window.inner_size();
                        if let Some(r) = &mut self.lui.renderer { r.resize(s.width, s.height); }
                        window.request_redraw();
                    }
                    _ => {}
                }
