...
fn text_box(text: &str, x: f32) -> LayoutBox {
  ...
  let glyphs = text
    .chars()
    .enumerate()
    .map(|(i, _)| lui_text::PositionedGlyph {
      x: i as f32 * 8.0,
      y: 0.0,
      w: 8.0,
      h: 16.0,
  ...
  LayoutBox {
    ...
    text_run: Some(lui_text::ShapedRun {
      ...
      text: text.to_owned(),
      byte_boundaries: lui_text::utf8_boundaries(text),
...
fn select_word_at_cursor_selects_token() {
  ...
  let mut tree = Tree::new(lui_tree::Node::new("hello, world"));
...
fn select_word_skips_invisible_chars_correctly() {
  ...
  let mut tree = Tree::new(lui_tree::Node::new(text));
...
fn select_all_spans_first_to_last_text_box() {
  ...
  let mut tree = Tree::new(lui_tree::Node::new("root"));
