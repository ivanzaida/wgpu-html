//! Top-level facade for the lui stack.
//!
...
pub fn compute_layout_profiled(
  ...
  let cascaded;
  {
    lui_tree::prof_scope!(&tree.profiler, "cascade");
    cascaded = lui_style::cascade_with_media(tree, &media);
  }
...
pub fn paint_tree_returning_layout_profiled(
  ...
  {
    lui_tree::prof_scope!(&tree.profiler, "paint");
...
pub struct PipelineCache {
  ...
  cascaded: Option<lui_style::CascadedTree>,
...
pub fn paint_tree_cached<'c>(
  ...
  match action {
    ...
    PipelineAction::PartialCascade => {
      ...
      let changed;
      {
        lui_tree::prof_scope!(&tree.profiler, "cascade_incremental");
        changed = if let Some(cascaded) = &mut cache.cascaded {
          lui_style::cascade_incremental_with_media(tree, cascaded, &old_snapshot, &media)
        } else {
          false
        };
      }
...
fn focused_input_value(tree: &Tree) -> Option<String> {
  ...
  match &node.element {
    lui_tree::Element::Input(inp) => inp.value.as_ref().map(|v| v.to_string()),
    _ => None,
  }
}
...
fn word_boundaries_for_cursor(run: &lui_text::ShapedRun, glyph_index: usize) -> Option<(usize, usize)> {
...
fn line_boundaries_for_cursor(run: &lui_text::ShapedRun, glyph_index: usize) -> (usize, usize) {
...
fn boundary_index_for_byte(run: &lui_text::ShapedRun, byte: usize) -> usize {
  run
    .byte_boundaries
    .iter()
    .position(|b| *b >= byte)
