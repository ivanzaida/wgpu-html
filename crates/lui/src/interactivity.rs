...
//! in `lui_tree::dispatch` and is also exposed as inherent
...
pub fn pointer_move(tree: &mut Tree, layout: &LayoutBox, pos: (f32, f32)) -> bool {
  ...
  if let Some(ref rd) = tree.interaction.range_drag.clone() {
    ...
    lui_tree::set_range_value_by_fraction(tree, &rd.path, frac);
  }
        ...
        match hit {
          date_picker_overlay::DatePickerHit::PrevMonth => {
            if let Some(dp) = &mut tree.interaction.date_picker {
              let (y, m) = lui_tree::date::prev_month(dp.view_year, dp.view_month);
              dp.view_year = y;
              dp.view_month = m;
            }
            return true;
          ...
          date_picker_overlay::DatePickerHit::NextMonth => {
            if let Some(dp) = &mut tree.interaction.date_picker {
              let (y, m) = lui_tree::date::next_month(dp.view_year, dp.view_month);
              dp.view_year = y;
              dp.view_month = m;
            }
            return true;
          ...
          date_picker_overlay::DatePickerHit::DayCell(row, col) => {
            ...
            if let Some(dp) = &mut tree.interaction.date_picker {
              ...
              let (iso, display) = if dp.has_time {
                let iso = lui_tree::date::format_datetime_local(y, m, d, dp.hour, dp.minute);
                let display = tree.locale.format_datetime(y, m, d, dp.hour, dp.minute);
                (iso, display)
              } else {
                let iso = lui_tree::date::format_date(y, m, d);
          ...
          date_picker_overlay::DatePickerHit::Reset => {
            if let Some(dp) = &mut tree.interaction.date_picker {
              let path = dp.path.clone();
              lui_tree::set_date_value(tree, &path, "");
    ...
    if tree.interaction.edit_cursor.is_some() {
      if let Some(focus_path) = tree.interaction.focus_path.clone() {
        ...
        if tree.interaction.date_display_value.is_some() {
          lui_tree::flush_date_to_iso(tree);
          let pattern = lui_tree::date::focused_date_pattern_from_tree(tree);
          let segs = lui_tree::date::parse_pattern_segments(&pattern);
          let clamped = lui_tree::date::clamp_to_editable(&segs, byte_offset);
          if let Some(seg_idx) = lui_tree::date::segment_at(&segs, clamped) {
            let s = &segs[seg_idx];
            tree.interaction.edit_cursor = Some(lui_tree::EditCursor {
        ...
        if let Some(ref fc) = lb.form_control {
          match fc.kind {
            FormControlKind::Range { min, max, .. } => {
              ...
              lui_tree::set_range_value_by_fraction(tree, target_path, frac);
            ...
            FormControlKind::Color { r, g, b, a } => {
              ...
              if already_open {
                ...
                let mut cp = ColorPickerState {
                  ...
                  field_cursor: lui_tree::EditCursor::collapsed(0),
                  field_blink_epoch: std::time::Instant::now(),
                  cached_layout: lui_tree::CachedLayout::default(),
                  layout_generation: 0,
                };
...
fn field_value(tree: &Tree, focus_path: &[usize]) -> Option<String> {
  tree
    ...
    .and_then(|node| match &node.element {
      lui_tree::Element::Input(inp) => Some(inp.value.as_deref().unwrap_or_default().to_string()),
      lui_tree::Element::Textarea(ta) => Some(ta.value.as_deref().unwrap_or_default().to_string()),
      _ => None,
    })
}

pub fn edit_cursor_for_click_count(value: &str, byte_offset: usize, click_count: u8) -> lui_tree::EditCursor {
  if click_count >= 3 {
    ...
    lui_tree::EditCursor::collapsed(byte_offset)
  }
}
