...
impl Component for StylesPanel {
    ...
    fn view(&self, props: &StylesPanelProps, _ctx: &Ctx<StylesPanelMsg>, env: &Tree) -> El {
        ...
        if let Some(node) = selected_node {
            // element.style rule
            let mut element_style = el::div().class("rule").children([
                el::div().class("rule-header").children([
            ...
            if let Some(style_str) = node.element.attr("style") {
                for decl in style_str.split(';') {
                    ...
                    if let Some((prop, value)) = decl.split_once(':') {
                        element_style =
                            element_style.child(make_decl_el(prop.trim(), value.trim()));
                    }
                }
            }
            element_style =
                element_style.child(el::div().class("rule-end").child(el::text("}")));
