...
fn shape_with_tiny_font_size() {
    ...
    let style = TextStyle { font_size: 0.1, line_height: 0.1, ..TextStyle::default() };
...
fn shape_with_very_large_font_size() {
    let mut ctx = FontContext::new();
    let style = TextStyle { font_size: 1000.0, line_height: 1200.0, ..TextStyle::default() };
...
fn shape_with_line_height_less_than_font_size() {
    let mut ctx = FontContext::new();
    let style = TextStyle { font_size: 24.0, line_height: 12.0, ..TextStyle::default() };
    let run = ctx.shape("Test", &style);
...
fn shape_empty_font_family() {
    let mut ctx = FontContext::new();
    let style = TextStyle { font_family: "", ..TextStyle::default() };
    let run = ctx.shape("test", &style);
