...
#[test]
fn uv_min_at_origin() {
    let entry = AtlasEntry { rect: AtlasRect { x: 0, y: 0, w: 10, h: 10 } };
...
#[test]
fn uv_min_at_offset() {
    let entry = AtlasEntry { rect: AtlasRect { x: 50, y: 25, w: 10, h: 10 } };
...
#[test]
fn uv_full_atlas() {
    let entry = AtlasEntry { rect: AtlasRect { x: 0, y: 0, w: 256, h: 256 } };
...
fn uv_min_always_less_than_uv_max() {
    let entry = AtlasEntry { rect: AtlasRect { x: 30, y: 40, w: 5, h: 8 } };
...
fn uv_values_are_in_unit_range() {
    let entry = AtlasEntry { rect: AtlasRect { x: 100, y: 100, w: 50, h: 50 } };
...
fn zero_size_entry_has_equal_min_max() {
    let entry = AtlasEntry { rect: AtlasRect { x: 10, y: 10, w: 0, h: 0 } };
