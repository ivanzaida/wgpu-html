...
fn shape_and_pack_cache_hit_returns_identical_dimensions() {
    ...
    let first = ctx.shape_and_pack("Hello", 16.0, 19.2, 400, color);
    let second = ctx.shape_and_pack("Hello", 16.0, 19.2, 400, color);
...
fn shape_and_pack_cache_hit_patches_color() {
    ...
    ctx.shape_and_pack("Test", 16.0, 19.2, 400, red);
    let cached = ctx.shape_and_pack("Test", 16.0, 19.2, 400, blue);
...
fn shape_and_pack_cache_hit_preserves_uv_coords() {
    let mut ctx = TextContext::new();
    let color = [1.0; 4];
    let first = ctx.shape_and_pack("UV", 16.0, 19.2, 400, color);
    let second = ctx.shape_and_pack("UV", 16.0, 19.2, 400, [0.0; 4]);
...
fn shape_and_pack_cache_hit_preserves_glyph_positions() {
    let mut ctx = TextContext::new();
    let first = ctx.shape_and_pack("Pos", 16.0, 19.2, 400, [1.0; 4]);
    let second = ctx.shape_and_pack("Pos", 16.0, 19.2, 400, [0.5; 4]);
...
fn shape_and_pack_cache_hit_preserves_text_and_char_data() {
    let mut ctx = TextContext::new();
    let color = [0.0; 4];
    let first = ctx.shape_and_pack("café", 16.0, 19.2, 400, color);
    let second = ctx.shape_and_pack("café", 16.0, 19.2, 400, color);
...
fn cache_miss_on_different_text() {
    let mut ctx = TextContext::new();
    let color = [0.0; 4];
    let a = ctx.shape_and_pack("AAA", 16.0, 19.2, 400, color);
    let b = ctx.shape_and_pack("BBB", 16.0, 19.2, 400, color);
...
fn cache_miss_on_different_font_size() {
    let mut ctx = TextContext::new();
    let color = [0.0; 4];
    let small = ctx.shape_and_pack("X", 12.0, 14.0, 400, color);
    let large = ctx.shape_and_pack("X", 24.0, 28.0, 400, color);
...
fn cache_miss_on_different_line_height() {
    let mut ctx = TextContext::new();
    let color = [0.0; 4];
    let a = ctx.shape_and_pack("X", 16.0, 19.2, 400, color);
    let b = ctx.shape_and_pack("X", 16.0, 32.0, 400, color);
...
fn cache_miss_on_different_weight() {
    let mut ctx = TextContext::new();
    let color = [0.0; 4];
    let normal = ctx.shape_and_pack("W", 16.0, 19.2, 400, color);
    let bold = ctx.shape_and_pack("W", 16.0, 19.2, 700, color);
...
fn color_does_not_affect_cache_key() {
    let mut ctx = TextContext::new();
    let a = ctx.shape_and_pack("C", 16.0, 19.2, 400, [1.0, 0.0, 0.0, 1.0]);
    let b = ctx.shape_and_pack("C", 16.0, 19.2, 400, [0.0, 1.0, 0.0, 1.0]);
...
fn cache_invalidated_when_font_registered() {
    ...
    let before = ctx.shape_and_pack("Test", 16.0, 19.2, 400, color);
    ...
    let after = ctx.shape_and_pack("Test", 16.0, 19.2, 400, color);
...
fn measure_run_cache_miss_on_different_font_size() {
    let mut ctx = TextContext::new();
    let fs_small = CssValue::Dimension { value: 12.0, unit: CssUnit::Px };
    let fs_large = CssValue::Dimension { value: 24.0, unit: CssUnit::Px };
    let small_style = ComputedStyle { font_size: Some(&fs_small), ..ComputedStyle::default() };
    let large_style = ComputedStyle { font_size: Some(&fs_large), ..ComputedStyle::default() };
...
fn cache_handles_many_distinct_entries() {
    ...
    for i in 0..100 {
        ...
        ctx.shape_and_pack(&text, 16.0, 19.2, 400, color);
    }

    // Verify first entry is still serviced (may be cache hit or reshaping)
    let run = ctx.shape_and_pack("entry_0", 16.0, 19.2, 400, color);
...
fn cache_still_works_after_overflow_clear() {
    ...
    let run = ctx.shape_and_pack("final", 16.0, 19.2, 400, color);
...
fn repeated_shape_and_pack_same_params_is_deterministic() {
    ...
    let runs: Vec<_> = (0..5)
        .map(|_| ctx.shape_and_pack("deterministic", 16.0, 19.2, 400, color))
        .collect();
