...
fn shape_run_with_custom_font_size() {
    let mut ctx = TextContext::new();
    let fs = CssValue::Dimension { value: 32.0, unit: CssUnit::Px };
...
fn shape_run_with_custom_weight() {
    ...
    let style = ComputedStyle { font_weight: Some(&fw), ..ComputedStyle::default() };
...
fn shape_run_with_custom_family() {
    ...
    let style = ComputedStyle { font_family: Some(&ff), ..ComputedStyle::default() };
...
fn pick_font_delegation_returns_none_without_registered_fonts() {
    ...
    assert!(ctx.pick_font(&["CustomFont"], 400, lui_glyph::FontStyleAxis::Normal).is_none());
}
