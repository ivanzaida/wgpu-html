...
impl FontContext {
    ...
    pub fn registry(&self) -> &FontRegistry { &self.registry }
    ...
    pub fn sync_registry(&mut self) {
        if self.registry.generation() == self.last_sync_gen
            && self.registry.len() == self.loaded.len()
        {
            return;
        }
        ...
        for (handle, face) in self.registry.iter() {
            ...
            let Some(&fontdb_id) = ids.first() else { continue; };

            self.loaded.insert(handle, LoadedFace { data_ptr: new_ptr, fontdb_id });
        }

        let stale: Vec<FontHandle> = self.loaded.keys()
            .copied()
    ...
    pub fn pick_font(&self, families: &[&str], weight: u16, style: FontStyleAxis) -> Option<FontHandle> {
        self.registry.find_first(families, weight, style)
    ...
    pub fn font_system_mut(&mut self) -> &mut FontSystem { &mut self.system }
}
