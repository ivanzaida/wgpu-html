...
impl TextContext {
  ...
  pub fn measure_run(&mut self, text: &str, style: &ComputedStyle) -> RunMetrics {
    ...
    let families: Vec<&str> = ts
      ...
      .filter(|s| !s.is_empty())
      .collect();
    let family_str = if families.is_empty() {
      "sans-serif"
    } else {
      ts.font_family
    };
  ...
  pub fn shape_and_pack(
    ...
    if let Some(cached) = self.text_cache.get(&cache_key) {
      let has_uvs = cached
        .glyphs
        .first()
        .is_some_and(|g| g.uv_min != [0.0; 2] || g.uv_max != [0.0; 2]);
      if has_uvs {
    ...
    for run in buffer.layout_runs() {
      ...
      for g in run.glyphs {
        ...
        glyphs.push(PositionedGlyph {
          ...
          x: gx,
          y: gy,
          w: entry.rect.w as f32 * inv,
          h: entry.rect.h as f32 * inv,
          uv_min,
          uv_max,
          color,
        });
