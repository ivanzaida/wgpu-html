...
impl Atlas {
    pub fn new(width: u32, height: u32) -> Self {
        ...
        Self { width, height, pixels, shelves: Vec::new(), next_shelf_y: 0, dirty: Vec::new() }
    }
    ...
    pub fn insert(&mut self, w: u32, h: u32, src: &[u8]) -> Option<AtlasEntry> {
        ...
        if w == 0 || h == 0 {
            return Some(AtlasEntry { rect: AtlasRect { x: 0, y: 0, w: 0, h: 0 } });
        }
        ...
        self.dirty.push(AtlasRect { x: rect.x - dx, y: rect.y - dy, w: rect.w + dx + dr, h: rect.h + dy + db });
    ...
    pub fn clear(&mut self) {
        ...
        self.next_shelf_y = 0;
        self.dirty.clear();
        self.dirty.push(AtlasRect { x: 0, y: 0, w: self.width, h: self.height });
    }
    ...
    fn allocate(&mut self, w: u32, h: u32) -> Option<AtlasRect> {
        ...
        if self.next_shelf_y + h + ATLAS_PAD > self.height { return None; }
        let shelf = Shelf { y: self.next_shelf_y, h, cursor_x: w + ATLAS_PAD };
    ...
    fn write_pixels(&mut self, rect: AtlasRect, src: &[u8]) {
        ...
        for row in 0..rect.h as usize {
            ...
            self.pixels[dst..dst + rect.w as usize]
                .copy_from_slice(&src[src_off..src_off + rect.w as usize]);
        }
        ...
        if x > 0 {
            let prev = x - 1;
            for row in y..y + h { self.pixels[row * stride + prev] = self.pixels[row * stride + x]; }
            if y > 0 { self.pixels[(y - 1) * stride + prev] = self.pixels[y * stride + x]; }
            if last_row + 1 < self.height as usize { self.pixels[(last_row + 1) * stride + prev] = self.pixels[last_row * stride + x]; }
        }
        if last_col + 1 < self.width as usize {
            let next = last_col + 1;
            for row in y..y + h { self.pixels[row * stride + next] = self.pixels[row * stride + last_col]; }
            if y > 0 { self.pixels[(y - 1) * stride + next] = self.pixels[y * stride + last_col]; }
            if last_row + 1 < self.height as usize { self.pixels[(last_row + 1) * stride + next] = self.pixels[last_row * stride + last_col]; }
        }
    }
