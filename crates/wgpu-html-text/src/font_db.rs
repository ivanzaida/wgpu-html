...
impl FontDb {
    ...
    /// - Faces whose `Arc<[u8]>` identity changed (re-registration in
    ///   place) are re-loaded; the previous fontdb entry is removed.
    /// - Faces in this cache that are no longer in the registry are
    ///   removed from both the local map and the fontdb (leaving any
    ///   shape-cache entries pointing at them stale — callers that
    ///   care should drop their `Buffer`s).
