...
impl PropertyGroup for TypographyGroup {
  ...
  fn apply(&self, declarations: &DeclarationBlock, style: &mut Style) {
    for decl in &declarations.declarations {
      match &*decl.property {
        "font-family" => style.font_family = Some(ArcStr::from(decl.value.as_ref())),
        "font-size" => style.font_size = parse_css_length(&decl.value),
        "font-weight" => style.font_weight = parse_font_weight(&decl.value),
        "font-style" => style.font_style = parse_font_style(&decl.value),
        "line-height" => style.line_height = parse_css_length(&decl.value),
        "letter-spacing" => style.letter_spacing = parse_css_length(&decl.value),
        "text-align" => style.text_align = parse_text_align(&decl.value),
        "text-decoration" => apply_text_decoration_shorthand(&decl.value, style),
        "text-transform" => style.text_transform = parse_text_transform(&decl.value),
        "text-overflow" => style.text_overflow = parse_text_overflow(&decl.value),
        "white-space" => apply_white_space_property(&decl.value, style),
        "word-break" => style.word_break = parse_word_break(&decl.value),
        "vertical-align" => style.vertical_align = parse_vertical_align(&decl.value),
        "font" => apply_font_shorthand(&decl.value, style),
        "font-variant" => apply_font_variant_shorthand(style, &decl.value),
        "font-variant-ligatures" => apply_font_variant_ligatures_shorthand(style, &decl.value),
        "font-synthesis" => apply_font_synthesis_shorthand(style, &decl.value),
        _ => {}
      }
    }
  }
