    ...
    for decl in &declarations.declarations {
      match &*decl.property {
        "grid-template-columns" => {
          let list = parse_grid_track_list(&decl.value);
          if !list.is_empty() {
        ...
        "grid-auto-columns" => style.grid_auto_columns = parse_grid_track_size(&decl.value),
        "grid-auto-rows" => style.grid_auto_rows = parse_grid_track_size(&decl.value),
        "grid-auto-flow" => style.grid_auto_flow = parse_grid_auto_flow(&decl.value),
        "grid-column" => apply_grid_axis_shorthand(&decl.value, style, GridAxis::Column),
        ...
        "grid-row" => apply_grid_axis_shorthand(&decl.value, style, GridAxis::Row),
        "grid-row-start" => style.grid_row_start = parse_grid_line(&decl.value),
        "grid-row-end" => style.grid_row_end = parse_grid_line(&decl.value),
        "justify-items" => style.justify_items = parse_justify_items(&decl.value),
        "justify-self" => style.justify_self = parse_justify_self(&decl.value),
        "grid" => apply_grid_shorthand(style, &decl.value),
        "grid-template" => apply_grid_template_shorthand(style, &decl.value),
        "grid-area" => apply_grid_area_shorthand(&decl.value, style),
        _ => {}
      }
    }
  }
