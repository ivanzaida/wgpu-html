...
pub fn tokenize(input: &str) -> Vec<Token> {
    ...
    while pos < chars.len() {
        ...
        // Number, percentage, dimension — or identifier
        let is_number_start = matches!(chars[pos], '+' | '-')
            && pos + 1 < chars.len()
            && (chars[pos + 1].is_ascii_digit() || chars[pos + 1] == '.');

        if is_number_start || chars[pos].is_ascii_digit() || (chars[pos] == '.' && pos + 1 < chars.len() && chars[pos + 1].is_ascii_digit()) {
            let start = pos;
            ...
            if pos < chars.len() && chars[pos] == '%' {
                ...
                tokens.push(Token::Dimension { value, unit: chars[unit_start..pos].iter().collect() });
            } else {
        ...
        // Fallback — anything else is a delimiter
        tokens.push(Token::Delim(chars[pos]));
        pos += 1;
    }
