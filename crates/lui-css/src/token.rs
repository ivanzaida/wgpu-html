...
impl<'a> Iterator for Tokenizer<'a> {
    ...
    fn next(&mut self) -> Option<Token<'a>> {
        ...
        Some(match b {
            ...
            b'\'' => self.consume_string(b'\''),
            b'#' => self.consume_hash(),
            b'(' => { self.advance(1); Token::OpenParen }
            b')' => { self.advance(1); Token::CloseParen }
            ...
            b';' => { self.advance(1); Token::Semicolon }
            ...
            b'+' | b'-' => {
                ...
                if next.is_some_and(|n| n.is_ascii_digit())
                    || (next == Some(&b'.') && self.input.as_bytes().get(self.pos + 2).is_some_and(|n| n.is_ascii_digit()))
                {
                    self.consume_numeric()
                } else if b == b'-' && next.is_some_and(|n| is_name_start(*n)) {
...
mod tests {
    ...
    #[test]
    fn tokenize_dimension_and_percentage() {
