...
use crate::images::{self, DecodedAsset, ImageData, SizedKey};
use crate::{AssetStatus, FetchConfig};
...
impl<F: Fetcher + 'static> AssetIo<F> {
    ...
    pub fn has_animated(&self) -> bool {
        self.decoded_images.values().any(|d| matches!(d, DecodedAsset::Animated { .. }))
    }
    ...
    pub fn preload_with(&mut self, url: &str, config: &FetchConfig) {
        ...
        self.cache.insert(url.to_owned(), CacheEntry {
    ...
    pub fn sweep(&mut self) {
        ...
        self.cache.retain(|url, entry| {
            let effective_ttl = entry.ttl_override
                .or(entry.max_age)
