...
fn decode_animated_webp(bytes: &[u8]) -> Option<DecodedAsset> {
    ...
    let decoded: Vec<DecodedFrameRaw> = frames
        ...
        .collect();
    Some(DecodedAsset::Animated { frames: Arc::new(decoded), w, h })
}
...
pub(crate) fn build_sized(src: &str, asset: &DecodedAsset, declared_w: Option<u32>, declared_h: Option<u32>) -> Option<ImageData> {
    match asset {
