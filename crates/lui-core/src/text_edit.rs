...
mod tests {
  ...
  fn insert_replaces_selection() {
    let cursor = EditCursor { cursor: 3, selection_anchor: Some(1) };
  ...
  fn move_right_collapses_selection() {
    let cursor = EditCursor { cursor: 1, selection_anchor: Some(3) };
  ...
  fn multibyte_backspace() {
    let (v, c) = delete_backward("a\u{00e9}b", &EditCursor::collapsed(3));
    assert_eq!(v, "ab");
    assert_eq!(c.cursor, 1);
  }
  ...
  fn multibyte_move_right() {
    let c = move_right("a\u{00e9}b", &EditCursor::collapsed(1), false);
    assert_eq!(c.cursor, 3);
  }
}
