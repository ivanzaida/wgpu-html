...
//!
//! This module defines the intermediate representation consumed by any
//! render backend. It contains no GPU types — only plain f32/u32
//! geometry and Arc<Vec<u8>> pixel data.
...
/// One quad. Two modes:
/// - **Filled**: `stroke == [0; 4]`. The whole box (with rounded corners from `radii_h` / `radii_v`) is filled with
///   `color`.
/// - **Stroked ring**: at least one `stroke` component > 0. The shader paints only the ring between the outer rounded
///   box and the inner one, where the inner one is inset on each side by the matching stroke width. `color` is used for
///   the entire ring.
...
impl DisplayList {
  ...
  pub fn pop_clip(&mut self, parent_rect: Option<Rect>, parent_radii_h: [f32; 4], parent_radii_v: [f32; 4]) {
    self.extend_open_range();
    let qs = self.quads.len() as u32;
    let is = self.images.len() as u32;
    let gs = self.glyphs.len() as u32;
  ...
  pub fn finalize(&mut self) {
    ...
    self.clips.retain(|r| {
      r.quad_range.0 != r.quad_range.1 || r.image_range.0 != r.image_range.1 || r.glyph_range.0 != r.glyph_range.1
    });
  ...
  pub fn push_quad_stroke(&mut self, rect: Rect, color: Color, radii: CornerRadii, stroke: StrokeWidths) -> &mut Self {
    let index = self.quads.len() as u32;
    self.quads.push(Quad {
      rect,
      color,
