use crate::{CssProperty, CssValue, type_keywords::keywords_for_type};
  ...
  match value {
    ...
    CssValue::Number(_) => {
      if syntax.contains("<number") || syntax.contains("<integer") {
        ...
        Validation::warn(format!("`{}` does not accept number values", property.name()))
      }
    }

    CssValue::Dimension { unit, .. } => {
      let unit_type = dimension_type(unit);
      if syntax.contains("<length") || syntax.contains("<dimension") || syntax.contains(unit_type) {
        Validation::ok()
      } else {
        Validation::warn(format!("`{}` does not accept dimension values", property.name()))
      }
    }
    ...
    CssValue::Percentage(_) => {
      if syntax.contains("<percentage") || syntax.contains("<length-percentage") {
        ...
        Validation::warn(format!("`{}` does not accept percentage values", property.name()))
      }
    }
    ...
    CssValue::Color(_) => {
      if syntax.contains("<color") {
        ...
        Validation::warn(format!("`{}` does not accept color values", property.name()))
      }
    }
    ...
    CssValue::Function { function, .. } => {
      ...
      if syntax.contains(&format!("<{}>", name)) || syntax.contains(&name) {
        ...
        Validation::warn(format!("`{}` function is not valid for `{}`", name, property.name()))
      }
    }

    CssValue::Var { .. } => Validation::ok(),

    CssValue::Url(_) => Validation::ok(),
  }
}
...
fn keyword_matches(kw: &str, syntax: &str) -> bool {
  ...
  for word in lower.split(|c: char| {
    c.is_ascii_whitespace() || c == '|' || c == '[' || c == ']' || c == '?' || c == '{' || c == '}' || c == ';'
  }) {
