...
pub enum CssFunction {
  ...
  /// prose: Implementations must accept -webkit-image-set() as a parse-time alias of image-set(). (It’s a valid value,
  /// with identical arguments to image-set(), and is turned into image-set() during parsing.)
  WebkitImageSet,
  ///
  ...
  /// prose: An absolutely positioned box can use the anchor-size() function in its sizing properties to refer to the
  /// size of one or more anchor boxes. The anchor-size() function resolves to a <length>. It is only allowed in the
  /// accepted @position-try properties (and is otherwise invalid).
  AnchorSize,
  ///
  /// href: https://drafts.csswg.org/css-anchor-position-1/#funcdef-anchor
  /// syntax: anchor( <anchor-name>? && <anchor-side>, <length-percentage>? )
  /// prose: An absolutely positioned box can use the anchor() function as a value in its inset properties to refer to
  /// the position of one or more anchor boxes. The anchor() function resolves to a <length>. It is only allowed in the
  /// inset properties (and is otherwise invalid).
  Anchor,
  ///
  ...
  /// prose: The attr() function substitutes the value of an attribute on an element into a property, similar to how the
  /// var() function substitutes a custom property value into a function.
  Attr,
  ///
  ...
  /// syntax: calc-interpolate( [ <progress-source> && [ by <easing-function> ]? && <easing-function>? ] ,
  /// <input-position>{1,2} : <calc-sum> , [ <easing-function>? , <input-position>{1,2} : <calc-sum> ]#? ) prose: The
  /// calc-interpolate() interpolation notation represents an interpolated numeric or dimensional value. Like calc(), it
  /// is a math function, with the following syntax:
  CalcInterpolate,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#funcdef-calc-mix
  /// syntax: calc-mix( [ <calc-sum> <percentage [0,100]>? ]# )
  /// prose: The calc-mix() mix notation represents a weighted average of numeric or dimensional value. Like calc(), it
  /// is a math function, with the following syntactic form:
  CalcMix,
  ///
  ...
  /// prose: The calc() function is a math function that allows basic arithmetic to be performed on numerical values,
  ...
  /// prose: The clamp() function takes three calculations—​a minimum value, a central value, and a maximum value—​and
  /// represents its central calculation, clamped according to its min and max calculations, favoring the min
  /// calculation if it conflicts with the max. (That is, given clamp(MIN, VAL, MAX), it represents exactly the same
  /// value as max(MIN, min(VAL, MAX))).
  Clamp,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#funcdef-color-interpolate
  /// syntax: color-interpolate( [ <progress-source> && [ by <easing-function> ]? && <easing-function>? &&
  /// <color-interpolation-method>? ] , <input-position>{1,2} : <color>, [ [ <easing-function> ||
  /// <color-interpolation-method> ]?, <input-position>{1,2} : <color> ]#? ) prose: The color-interpolate()
  /// interpolation notation represents an interpolated <color> value, with the following syntax:
  ColorInterpolate,
  ///
  /// href: https://drafts.csswg.org/css-color-6/#funcdef-color-layers
  /// syntax: color-layers([ <blend-mode>, ]? <color># )
  /// prose: The color-layers() functional notation takes an optional <blend-mode> followed by a list of two or more
  /// <color> layers.
  ColorLayers,
  ///
  ...
  /// syntax: contrast-color( [ [ <color> && [ tbd-fg | tbd-bg ] && <target-contrast>? ] | [ <color> && [ tbd-fg |
  /// tbd-bg ] && <target-contrast>, <color># ] ] ) prose: The contrast-color() functional notation identifies a
  /// sufficiently contrasting color against a specified background or foreground color without requiring manual
  /// computation.
  ContrastColor,
  ///
  ...
  /// prose: The control-value() function computes to the current value of the form control it is on. If it is used on
  /// an element that is not a form control, it returns an empty string.
  ControlValue,
  ///
  ...
  /// prose: Counters have no visible effect by themselves, but their values can be used with the counter() and
  /// counters() functions, whose used values represent counter values as strings or images. They are defined as
  /// follows:
  Counter,
  ///
  ...
  /// prose: The first-valid() functional notation inlines the fallback behavior intrinsic to parsing declarations.
  /// Unlike most notations, it can accept any valid or invalid syntax in its arguments, and represents the first value
  /// among its arguments that is supported (parsed as valid) by the UA as the whole value of the property it’s used in.
  FirstValid,
  ///
  ...
  /// syntax: hwb([from <color>]? [<hue> | none] [<percentage> | <number> | none] [<percentage> | <number> | none] [ /
  /// [<alpha-value> | none] ]? )
  Hwb,
  ///
  ...
  /// syntax: ictcp([from <color>]? [<percentage> | <number> | none] [<percentage> | <number> | none] [<percentage> |
  /// <number> | none] [ / [<alpha-value> | none] ]? )
  Ictcp,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#funcdef-ident
  /// syntax: ident( <ident-arg>+ )
  /// prose: The ident() function represents an <ident>, and can be used to manually construct <custom-ident> values
  /// from several parts.
  Ident,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#funcdef-if
  /// syntax: if( [ <if-branch> ; ]* <if-branch> ;? )
  /// prose: The if() function is an arbitrary substitution function that represents conditional values. Its argument
  /// consists of an ordered semi-colon–separated list of statements, each consisting of a condition followed by a colon
  /// followed by a value. An if() function represents the value corresponding to the first condition in its argument
  /// list to be true; if no condition matches, then the if() function represents an empty token stream.
  If,
  ///
  /// href: https://drafts.csswg.org/css-images-4/#funcdef-image-set
  /// syntax: image-set( <image-set-option># )
  /// prose: Delivering the most appropriate image resolution for a user’s device can be a difficult task. Ideally,
  /// images should be in the same resolution as the device they’re being viewed in, which can vary between users.
  /// However, other factors can factor into the decision of which image to send; for example, if the user is on a slow
  /// mobile connection, they may prefer to receive lower-res images rather than waiting for a large proper-res image to
  /// load. The image-set() function allows an author to ignore most of these issues, simply providing multiple
  /// resolutions of an image and letting the UA decide which is most appropriate in a given situation.
  ImageSet,
  ///
  ...
  /// prose: Like the inherit keyword, the inherit() functional notation resolves to the computed value of a property on
  /// the parent. Rather than resolving to the value of the same property, however, it resolves to a sequence of
  /// component values representing the computed value of the property specified as its first argument. Its second
  /// argument, if present, is used as a fallback in case the first argument resolves to the guaranteed-invalid value.
  Inherit,
  ///
  ...
  /// syntax: interpolate( [ <progress-source> && [ by <easing-function> ]? && <easing-function>? ] ,
  /// <input-position>{1,2} : <whole-value>, [ <easing-function>?, <input-position>{1,2} : <whole-value> ]#? ) |
  /// interpolate( <progress-source> && [ by <easing-function> ]? && <easing-function>? of <keyframes-name> )
  /// prose: The interpolate() interpolation notation represents the interpolation of entire property values, which
  /// supports two alternative syntax patterns:
  Interpolate,
  ///
  ...
  /// syntax: jzazbz([from <color>]? [<percentage> | <number> | none] [<percentage> | <number> | none] [<percentage> |
  /// <number> | none] [ / [<alpha-value> | none] ]? )
  Jzazbz,
  ///
  /// href: https://drafts.csswg.org/css-color-hdr-1/#funcdef-jzczhz
  /// syntax: jzczhz([from <color>]? [<percentage> | <number> | none] [<percentage> | <number> | none] [<hue> | none] [
  /// / [<alpha-value> | none] ]? )
  Jzczhz,
  ///
  /// href: https://drafts.csswg.org/css-color-5/#funcdef-lab
  /// syntax: lab([from <color>]? [<percentage> | <number> | none] [<percentage> | <number> | none] [<percentage> |
  /// <number> | none] [ / [<alpha-value> | none] ]? )
  Lab,
  ///
  /// href: https://drafts.csswg.org/css-color-5/#funcdef-lch
  /// syntax: lch([from <color>]? [<percentage> | <number> | none] [<percentage> | <number> | none] [<hue> | none] [ /
  /// [<alpha-value> | none] ]? )
  Lch,
  ///
  ...
  /// syntax: max( <calc-sum># )
  /// prose: The min() or max() functions contain one or more comma-separated calculations, and represent the smallest
  ...
  /// syntax: oklab([from <color>]? [<percentage> | <number> | none] [<percentage> | <number> | none] [<percentage> |
  /// <number> | none] [ / [<alpha-value> | none] ]? )
  Oklab,
  ///
  /// href: https://drafts.csswg.org/css-color-5/#funcdef-oklch
  /// syntax: oklch([from <color>]? [<percentage> | <number> | none] [<percentage> | <number> | none] [<hue> | none] [ /
  /// [<alpha-value> | none] ]? )
  Oklch,
  ///
  ...
  /// syntax: palette-mix(<color-interpolation-method> , [ [normal | light | dark | <palette-identifier> |
  /// <palette-mix()> ] && <percentage [0,100]>? ]#{2}) prose: With the palette-mix() function defined as follows:
  PaletteMix,
  ///
  /// href: https://drafts.csswg.org/css-link-params-1/#funcdef-param
  /// syntax: param( <dashed-ident> , <declaration-value>? )
  /// prose: The param() function specifies a link parameter, with a key of the <dashed-ident>, and a value of the
  /// <declaration-value>?. (If the <declaration-value> is omitted, it represents an empty value.) It has the syntax:
  Param,
  ///
  ...
  /// prose: The pointer() functional notation can be used as a <single-animation-timeline> value in animation-timeline
  /// and specifies a pointer progress timeline. Its syntax is
  Pointer,
  ///
  ...
  /// prose: The progress() functional notation represents the proportional distance of a given value (the progress
  /// value) from one value (the progress start value) to another value (the progress end value), each represented as a
  /// calculation. It is a math function, and can be input into other calculations such as a math function or a mix
  /// notation.
  Progress,
  ///
  /// href: https://drafts.csswg.org/css-images-3/#funcdef-radial-gradient
  /// syntax: radial-gradient( [ <radial-gradient-syntax> ] )
  /// prose: The radial-gradient() notation specifies a radial gradient by indicating the center of the gradient (where
  /// the 0% ellipse will be) and the size and shape of the ending shape (the 100% ellipse). Color stops are given as a
  /// list, just as for linear-gradient(). Starting from the gradient center and progressing towards (and potentially
  /// beyond) the ending shape, uniformly-scaled concentric ellipses are drawn and colored according to the specified
  /// color stops.
  RadialGradient,
  ///
  ...
  /// prose: The random() function is a math function that represents a random value between a minimum and maximum
  /// value, drawn from a uniform distribution, optionally limiting the possible values to a step between those limits:
  Random,
  ///
  ...
  /// prose: <referrer-policy-modifier> = referrer-policy(no-referrer | no-referrer-when-downgrade | same-origin |
  /// origin | strict-origin | origin-when-cross-origin | strict-origin-when-cross-origin | unsafe-url) for_parents:
  ...
  /// prose: Such a list may contain repeat() notations. Similar to CSS Grid Layout 1 § 7.2.3 Repeating Rows and
  /// Columns: the repeat() notation, these notations allow a series of gap decorations that exhibit a recurring pattern
  /// to be written in a more compact form. for_parents: [<auto-repeat-line-color>, <auto-repeat-line-style>,
  /// <auto-repeat-line-width>, <repeat-line-color>, <repeat-line-style>, <repeat-line-width>]
  RepeatFuncdefRepeatLineColorRepeat,
  ///
  /// href: https://drafts.csswg.org/css-grid-2/#funcdef-track-repeat-repeat
  /// prose: The repeat() notation represents a repeated fragment of the track list, allowing a large number of columns
  /// or rows that exhibit a recurring pattern to be written in a more compact form. for_parents: [<auto-repeat>,
  /// <fixed-repeat>, <track-repeat>]
  ...
  /// prose: In addition to linear-gradient(), radial-gradient(), and conic-gradient(), this specification defines
  /// repeating-linear-gradient(), repeating-radial-gradient(), and repeating-conic-gradient() values. These notations
  /// take the same values and are interpreted the same as their respective non-repeating siblings defined previously.
  ...
  /// prose: The scroll() functional notation can be used as a <single-animation-timeline> value in animation-timeline
  /// and specifies a scroll progress timeline. Its syntax is
  Scroll,
  ///
  ...
  /// prose: The sibling-count() functional notation represents, as an <integer>, the total number of child elements in
  /// the parent of the element on which the notation is used.
  SiblingCount,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#funcdef-sibling-index
  /// prose: The sibling-index() functional notation represents, as an <integer>, the index of the element on which the
  ...
  /// prose: The <url> type, written with the url() and src() functions, represents a URL, which is a pointer to a
  /// resource.
  Src,
  ///
  ...
  /// prose: The stripes() function defines a 1D image as a comma-separated list of colored stripes, each placed
  /// end-to-end on the paint line in the order given.
  Stripes,
  ///
  ...
  /// prose: The toggle() expression allows descendant elements to cycle over a list of values instead of inheriting the
  /// same value.
  Toggle,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#funcdef-transform-interpolate
  /// syntax: transform-interpolate( [ <progress-source> && [ by <easing-function> ]? && <easing-function>? ],
  /// <input-position>{1,2} : <transform-list>, [ <easing-function>?, <input-position>{1,2} : <transform-list> ]#? )
  /// prose: The transform-interpolate() interpolation notation represents an interpolated <transform-list>, with the
  /// following syntax:
  ...
  /// prose: The transform-mix() mix notation represents a weighted average of <transform-list>, with the following
  /// syntactic form:
  TransformMix,
  ///
  ...
  /// prose: The view() functional notation can be used as a <single-animation-timeline> value in animation-timeline and
  /// specifies a view progress timeline in reference to the nearest ancestor scroll container. Its syntax is
  View,
  ///
  /// href: https://drafts.csswg.org/css-color-6/#funcdef-contrast-color-wcag2
  /// prose: The wcag2 keyword and wcag2() functional notations indicate use of the [WCAG21] luminance contrast
  /// algorithm. Their syntax is: for_parents: [contrast-color()]
  Wcag2,
  ///
