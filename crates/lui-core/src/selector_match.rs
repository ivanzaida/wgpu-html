...
fn matches_pseudo(
  ...
  match &pseudo.pseudo {
    ...
    CssPseudo::Not => match &pseudo.arg {
      Some(arg) => match crate::selector_parse::parse_selector_list(arg) {
        Ok(inner) => !inner
          .0
          .iter()
...
fn match_has(node: &HtmlNode, arg: &str) -> bool {
  if let Some(rest) = arg.strip_prefix('>') {
    match crate::selector_parse::parse_selector_list(rest.trim()) {
    ...
    match crate::selector_parse::parse_selector_list(arg) {
