...
pub enum CssType {
  ...
  /// prose: The output of tokenization step is a stream of zero or more of the following tokens: <ident-token>,
  /// <function-token>, <at-keyword-token>, <hash-token>, <string-token>, <bad-string-token>, <url-token>,
  /// <bad-url-token>, <delim-token>, <number-token>, <percentage-token>, <dimension-token>, <unicode-range-token>,
  /// <whitespace-token>, <CDO-token>, <CDC-token>, <colon-token>, <semicolon-token>, <comma-token>, <[-token>,
  ...
  /// prose: Possible values are child, young and old, indicating the preferred age category to match during voice
  /// selection. for_parents: [voice-family]
  Age,
  ///
  ...
  /// syntax: odd | even | <integer> | <n-dimension> | '+'? n | -n | <ndashdigit-dimension> | '+'? <ndashdigit-ident> |
  /// <dashndashdigit-ident> | <n-dimension> <signed-integer> | '+'? n <signed-integer> | -n <signed-integer> |
  /// <ndash-dimension> <signless-integer> | '+'? n- <signless-integer> | -n- <signless-integer> | <n-dimension> ['+' |
  /// '-'] <signless-integer> | '+'? n ['+' | '-'] <signless-integer> | -n ['+' | '-'] <signless-integer>
  Anb,
  ///
  ...
  /// syntax: inside | outside | top | left | right | bottom | start | end | self-start | self-end | <percentage> |
  /// center
  AnchorSide,
  ///
  ...
  /// syntax: not <anchored-in-parens> | <anchored-in-parens> [ [ and <anchored-in-parens> ]* | [ or
  /// <anchored-in-parens> ]* ] | <anchored-feature>
  AnchoredQuery,
  ///
  ...
  /// prose: The <any-value> production is identical to <declaration-value>, but also allows top-level <semicolon-token>
  /// tokens and <delim-token> tokens with a value of "!". It represents the entirety of what valid CSS can be in any
  /// context.
  AnyValue,
  ///
  /// href: https://drafts.csswg.org/css-shapes-1/#typedef-shape-arc-command
  /// syntax: arc <command-end-point> [ [ of <length-percentage>{1,2} ] && <arc-sweep>? && <arc-size>? && [rotate
  /// <angle>]? ] for_parents: [shape()]
  ArcCommand,
  ///
  /// href: https://drafts.csswg.org/css-shapes-1/#typedef-shape-arc-size
  /// syntax: large | small
  /// prose: <arc-size> can be large or small, indicating that the larger or smaller, respectively, of the two possible
  /// arcs must be chosen. If omitted, this defaults to small. for_parents: [shape()]
  ArcSize,
  ///
  /// href: https://drafts.csswg.org/css-shapes-1/#typedef-shape-arc-sweep
  /// syntax: cw | ccw
  /// prose: <arc-sweep> can be cw or ccw, indicating that the arc that is traced around the ellipse clockwise or
  /// counter-clockwise from the center, respectively, must be chosen. If omitted, this defaults to ccw. for_parents:
  /// [shape()]
  ArcSweep,
  ///
  ...
  /// syntax: [ <line-names>? [ <fixed-size> | <fixed-repeat> ] ]* <line-names>? <auto-repeat> [ <line-names>? [
  /// <fixed-size> | <fixed-repeat> ] ]* <line-names>?
  AutoTrackList,
  ///
  ...
  /// prose: The <basic-shape> type can be specified using basic shape functions. When using this syntax to define
  /// shapes, the reference box is defined by each property that uses <basic-shape> values. The coordinate system for
  /// the shape has its origin on the top-left corner of the reference box with the x-axis running to the right and the
  /// y-axis running downwards. All the lengths expressed in percentages are resolved from the used dimensions of the
  /// reference box.
  BasicShape,
  ///
  ...
  /// syntax: <bg-image> || <bg-position> [ / <bg-size> ]? || <repeat-style> || <attachment> || <bg-clip> ||
  /// <visual-box>
  BgLayer,
  ///
  /// href: https://drafts.csswg.org/css-backgrounds-4/#typedef-bg-position
  /// syntax: <position> | <position-three>
  /// prose: Its value is given as a comma-separated list of <bg-position> values, which are interpreted as <position>
  /// values with the resized background image as the alignment subject and the background positioning area as the
  /// alignment container.
  BgPosition,
  ///
  ...
  /// syntax: normal | darken | multiply | color-burn | lighten | screen | color-dodge | overlay | soft-light |
  ...
  /// prose: When writing a rule grammar, <block-contents> represents this agnostic parsing. It must only be used as the
  /// sole value in a block, and represents that no restrictions are implicitly placed on what the block can contain.
  BlockContents,
  ///
  /// href: https://drafts.csswg.org/css-conditional-5/#typedef-boolean-condition
  /// prose: Where <boolean-condition> is a boolean algebra a la Media Queries 4 § 3 Syntax, but with media() and
  /// supports() functions as leaves.
  BooleanCondition,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#typedef-boolean-expr
  /// syntax: not <boolean-expr-group> | <boolean-expr-group> [ [ and <boolean-expr-group> ]* | [ or
  /// <boolean-expr-group> ]* ] prose: Several contexts (such as @media, @supports, if(), ...) specify conditions, and
  /// allow combining those conditions with boolean logic (and/or/not/grouping). Because they use the same non-trivial
  /// recursive syntax structure, the special <boolean-expr> production represents this pattern generically.
  BooleanExpr,
  ///
  ...
  /// prose: The border style properties specify the line style of a box’s border (solid, double, dashed, etc.). The
  /// properties defined in this section refer to the <border-style> value type, which may take one of the following
  /// values:
  BorderStyle,
  ///
  /// href: https://drafts.csswg.org/css2/#value-def-border-width
  /// prose: The border width properties specify the width of the border area. The properties defined in this section
  /// refer to the <border-width> value type, which may take one of the following values:
  BorderWidth,
  ///
  /// href: https://drafts.csswg.org/css2/#value-def-bottom
  /// prose: <top>, <right>, <bottom>, and <left> may either have a <length> value or auto. Negative lengths are
  /// permitted. The value auto means that a given edge of the clipping region will be the same as the edge of the
  /// element’s generated border box (i.e., auto means the same as 0 for <top> and <left>, the same as the used value of
  /// the height plus the sum of vertical padding and border widths for <bottom>, and the same as the used value of the
  /// width plus the sum of the horizontal padding and border widths for <right>, such that four auto values result in
  /// the clipping region being the same as the element’s border box).
  Bottom,
  ///
  /// href: https://drafts.csswg.org/css-box-4/#typedef-box
  /// prose: The following <box> CSS keywords are defined for use in properties (such as transform-box and
  ...
  /// syntax: <rgb()> | <rgba()> | <hsl()> | <hsla()> | <hwb()> | <lab()> | <lch()> | <oklab()> | <oklch()> | <ictcp()>
  /// | <jzazbz()> | <jzczhz()> | <alpha()> | <color()> | <hdr-color()>
  ColorFunction,
  ///
  ...
  /// prose: Each <color-stripe> entry defines a solid-color stripe with the specified <color> and thickness. If the
  /// thickness is omitted, it defaults to 1fr. Thickness values are interpreted as follows:
  ColorStripe,
  ///
  ...
  /// syntax: clear | copy | source-over | destination-over | source-in | destination-in | source-out | destination-out
  /// | source-atop | destination-atop | xor | lighter | plus-darker | plus-lighter
  CompositeMode,
  ///
  ...
  /// syntax: [ [ [ from [ <angle> | <zero> ] ]? [ at <position> ]? ] || <color-interpolation-method> ]? ,
  ...
  /// syntax: [ <string> | <image> | <attr()> | contents | <quote> | <leader()> | <target> | <string()> | <content()> |
  /// <counter> ]+ prose: Replaces the element’s contents with one or more anonymous inline boxes corresponding to the
  /// specified values, in the order specified. Its normal contents are suppressed and do not generate boxes, as if they
  /// were display: none. Each value contributes an inline box to the element’s contents. For <image>, this is an inline
  /// anonymous replaced element; for the others, it’s an anonymous inline run of text. If an <image> represents an
  /// invalid image, the user agent must do one of the following: "Skip" the <image>, generating nothing for it. Display
  /// some indication that the image can’t be displayed in place of the <image>, such as a "broken image" icon. This
  /// specification intentionally does not define which behavior a user agent must use, but it must use one or the other
  /// consistently. for_parents: [content]
  ...
  /// syntax: <custom-ident>
  /// prose: Counters are referred to in CSS syntax using the <counter-name> type, which represents their name as a
  /// <custom-ident>. A <counter-name> name cannot match the keyword none; such an identifier is invalid as a
  /// <counter-name>.
  CounterName,
  ///
  ...
  /// syntax: <custom-ident>
  /// prose: <counter-style-name> is a <custom-ident> that is not an ASCII case-insensitive match for none. When used
  /// here, to define a counter style, it also cannot be any of the non-overridable counter-style names (in other uses
  /// that merely reference a counter style, such as the extend system, these are allowed). The <counter-style-name> is
  /// a tree-scoped name.
  CounterStyleName,
  ///
  ...
  /// syntax: auto | default | none | context-menu | help | pointer | progress | wait | cell | crosshair | text |
  /// vertical-text | alias | copy | move | no-drop | not-allowed | grab | grabbing | e-resize | n-resize | ne-resize |
  ...
  /// syntax: curve [ [ to <position> with <control-point> [ / <control-point> ]? ] | [ by <coordinate-pair> with
  /// <relative-control-point> [ / <relative-control-point> ]? ] ] for_parents: [shape()]
  CurveCommand,
  ///
  ...
  /// prose: Some properties accept arbitrary author-defined identifiers as a component value. This generic data type is
  /// denoted by <custom-ident>, and represents any valid CSS identifier that would not be misinterpreted as a
  /// pre-defined keyword in that property’s value definition. Such identifiers are fully case-sensitive (meaning
  /// they’re compared using the "identical to" operation), even in the ASCII range (e.g. example and EXAMPLE are two
  /// different, unrelated user-defined identifiers).
  CustomIdent,
  ///
  ...
  /// prose: A custom property is any property whose name starts with two dashes (U+002D HYPHEN-MINUS), like --foo. The
  /// <custom-property-name> production corresponds to this: it’s defined as any <dashed-ident> (a valid identifier that
  /// starts with two dashes), except -- itself, which is reserved for future use by CSS. Custom properties are solely
  /// for use by authors and users; CSS will never give them a meaning beyond what is presented here.
  CustomPropertyName,
  ///
  ...
  /// prose: A <dashed-function> is a functional notation whose function name starts with two dashes (U+002D
  /// HYPHEN-MINUS). Its argument grammar is:
  DashedFunction,
  ///
  /// href: https://drafts.csswg.org/css-values-4/#typedef-dashed-ident
  /// prose: The <dashed-ident> production is a <custom-ident>, with all the case-sensitivity that implies, with the
  ...
  /// prose: The <decibel> type denotes a dimension with a "dB" (decibel unit) unit identifier. Decibels represent the
  /// ratio of the squares of the new signal amplitude a1 and the current amplitude a0, as per the following logarithmic
  /// equation: volume(dB) = 20 × log10(a1 / a0). for_parents: [voice-volume]
  Decibel,
  ///
  /// href: https://drafts.csswg.org/css-conditional-5/#typedef-declaration
  /// prose: <declaration> here matches anything that would be successfully parsed by consume a declaration, ignoring
  /// the context-validation check at the end of that algorithm. Notably, this includes a trailing !important, which is
  /// valid but ignored for the purpose of @supports.
  Declaration,
  ///
  ...
  /// prose: The <declaration-value> production matches any sequence of one or more tokens, so long as the sequence does
  ...
  /// syntax: ActiveBorder | ActiveCaption | AppWorkspace | Background | ButtonHighlight | ButtonShadow | CaptionText |
  ...
  /// WindowText prose: The deprecated system colors are represented as the <deprecated-color> sub-type, and are
  /// defined as:
  DeprecatedColor,
  ///
  ...
  /// syntax: table-row-group | table-header-group | table-footer-group | table-row | table-cell | table-column-group |
  ...
  /// prose: An <eof-token> is a conceptual token, not actually produced by the tokenizer, used to indicate that the
  ...
  /// prose: All extensions defined in this specification use a common syntax for defining their ”names”: the
  /// <extension-name> production. An <extension-name> is any identifier that starts with two dashes (U+002D
  /// HYPHEN-MINUS), like --foo, or even exotic names like -- or ------. The CSS language will never use identifiers of
  /// this form for any language-defined purpose, so it’s safe to use them for author-defined purposes without ever
  /// having to worry about colliding with CSS-defined names.
  ExtensionName,
  ///
  ...
  /// syntax: <blur()> | <brightness()> | <contrast()> | <drop-shadow()> | <grayscale()> | <hue-rotate()> | <invert()> |
  ...
  /// syntax: <fixed-breadth> | minmax( <fixed-breadth> , <track-breadth> ) | minmax( <inflexible-breadth> ,
  /// <fixed-breadth> )
  FixedSize,
  ///
  /// href: https://drafts.csswg.org/css-grid-2/#typedef-flex
  /// prose: A flexible length or <flex> is a dimension with the fr unit, which represents a fraction of the leftover
  /// space in the grid container. Tracks sized with fr units are called flexible tracks as they flex in response to
  /// leftover space similar to how flex items with a zero base size fill space in a flex container.
  Flex,
  ///
  /// href: https://drafts.csswg.org/css-fonts-4/#font-family-name-value
  /// syntax: <string> | <custom-ident>+
  /// prose: The name of a font family, such as Helvetica or Verdana in the previous example. This might be a
  /// locally-instaled font, or might be a web font.
  FontFamilyName,
  ///
  ...
  /// syntax: normal | ultra-condensed | extra-condensed | condensed | semi-condensed | semi-expanded | expanded |
  /// extra-expanded | ultra-expanded
  FontWidthCss3,
  ///
  /// href: https://drafts.csswg.org/selectors-4/#typedef-forgiving-selector-list
  /// prose: The <forgiving-selector-list> production instead parses each selector in the list individually, simply
  /// ignoring ones that fail to parse, so the remaining selectors can still be used.
  ...
  /// prose: In the example above, the last value is a generic family name. The following generic families are defined:
  /// serif (e.g., Times) sans-serif (e.g., Helvetica) cursive (e.g., Zapf-Chancery) fantasy (e.g., Western) monospace
  /// (e.g., Courier) Style sheet designers are encouraged to offer a generic font family as a last alternative. Generic
  /// font family names are keywords and must NOT be quoted.
  GenericFamily,
  ///
  ...
  /// syntax: [ <linear-gradient()> | <repeating-linear-gradient()> | <radial-gradient()> |
  ...
  /// syntax: auto | <custom-ident> | [ [ <integer [-∞,-1]> | <integer [1,∞]> ] && <custom-ident>? ] | [ span && [
  /// <integer [1,∞]> || <custom-ident> ] ] for_parents: [grid-column-end, grid-column-start, grid-row-end,
  /// grid-row-start]
  GridLine,
  ///
  ...
  /// prose: The syntax of a <hex-color> is a <hash-token> token whose value consists of 3, 4, 6, or 8 hexadecimal
  /// digits. In other words, a hex color is written as a hash character, "#", followed by some number of digits 0-9 or
  ...
  /// prose: The <id> value is an ID selector [SELECT]. In response to directional navigation input corresponding to the
  /// property, the focus is navigated to the first element in tree order matching the selector. If this refers to the
  /// currently focused element, the directional navigation input respective to the nav- property is ignored — there is
  /// no need to refocus the same element. If no element matches the selector, the user agent automatically determines
  /// which element to navigate the focus to. If the focus is navigated to an element that was not otherwise focusable,
  /// it becomes focusable only as the result of this directional navigation, and the :focus pseudo-class matches the
  /// element while it is focused as such.
  Id,
  ///
  ...
  /// prose: CSS identifiers, generically denoted by <ident>, consist of a sequence of characters conforming to the
  /// <ident-token> grammar. [CSS-SYNTAX-3] Identifiers cannot be quoted; otherwise they would be interpreted as
  /// strings. CSS properties accept two classes of identifiers: pre-defined keywords and author-defined identifiers.
  Ident,
  ///
  ...
  /// syntax: supports( [ <ident> : <declaration-value> ] | <supports-condition> ) | media( <media-feature> |
  /// <media-condition> ) | style( <style-query> )
  IfTest,
  ///
  ...
  /// syntax: <stripes()>
  /// prose: While <image> values represent a 2-dimensional (2D) image, and <color> can be thought of as a 0-dimensional
  /// (0D) image (unvarying in either axis), some contexts require a 1-dimensional (1D) image, which specifies colors
  /// along an abstract, directionless, single-axis paint line. The <image-1D> type represents such 1D images, including
  /// the stripes() functional notation:
  Image1d,
  ///
  ...
  /// for_parents: [calc-interpolate(), calc-interpolate(), color-interpolate(), color-interpolate(), interpolate(),
  /// interpolate(), transform-interpolate(), transform-interpolate()]
  InputPosition,
  ///
  ...
  /// prose: Lengths refer to distance measurements and are denoted by <length> in the property definitions. A length is
  /// a dimension.
  Length,
  ///
  ...
  /// prose: The properties defined in this section refer to the <margin-width> value type, which may take one of the
  /// following values:
  MarginWidth,
  ///
  ...
  /// syntax: <mask-reference> || <position> [ / <bg-size> ]? || <repeat-style> || <geometry-box> || [ <geometry-box> |
  ...
  /// prose: To parse a <media-query-list> production, parse a comma-separated list of component values, then parse each
  /// entry in the returned list as a <media-query>. Its value is the list of <media-query>s so produced.
  MediaQueryList,
  ///
  ...
  /// syntax: <mf-name> <mf-comparison> <mf-value> | <mf-value> <mf-comparison> <mf-name> | <mf-value> <mf-lt> <mf-name>
  /// <mf-lt> <mf-value> | <mf-value> <mf-gt> <mf-name> <mf-gt> <mf-value>
  MfRange,
  ///
  ...
  /// syntax: hsl([from <color>]? [<hue> | none] [<percentage> | <number> | none] [<percentage> | <number> | none] [ /
  /// [<alpha-value> | none] ]? )
  ModernHslSyntax,
  ///
  /// href: https://drafts.csswg.org/css-color-5/#typedef-modern-hsla-syntax
  /// syntax: hsla([from <color>]? [<hue> | none] [<percentage> | <number> | none] [<percentage> | <number> | none] [ /
  /// [<alpha-value> | none] ]? )
  ModernHslaSyntax,
  ///
  ...
  /// syntax: aliceblue | antiquewhite | aqua | aquamarine | azure | beige | bisque | black | blanchedalmond | blue |
  ...
  /// | yellowgreen | transparent prose: CSS defines a large set of named colors, so that common colors can be written
  /// and read more easily. A <named-color> is written as an <ident>, accepted anywhere a <color> is. As usual for
  /// CSS-defined <ident>s, all of these keywords are ASCII case-insensitive.
  NamedColor,
  ///
  ...
  /// syntax: not <navigation-in-parens> | <navigation-in-parens> [ and <navigation-in-parens> ]* |
  ...
  /// syntax: <number> | <percentage>
  /// prose: The opacity to be applied to the element. The resulting opacity is applied to the entire element, rather
  /// than a particular color. Opacity values outside the range [0,1] are not invalid, and are preserved in specified
  /// values, but are clamped to the range [0, 1] in computed values. for_parents: [opacity]
  OpacityValue,
  ///
  ...
  /// prose: The properties defined in this section refer to the <padding-width> value type, which may take one of the
  /// following values:
  PaddingWidth,
  ///
  ...
  /// prose: A page size can be specified using one of the following media names. This is the equivalent of specifying
  /// size using length values. The definition of the media names comes from Media Standardized Names [PWGMSN].
  /// for_parents: [size]
  PageSize,
  ///
  ...
  /// syntax: <dashed-ident>
  /// prose: This value identifies an CSS-defined palette to use. Users can define a palette by using the
  /// @font-palette-values rule. If no applicable @font-palette-values rule is present, this value behaves as normal.
  /// <palette-identifier> is parsed as a <dashed-ident>. for_parents: [font-palette]
  PaletteIdentifier,
  ///
  ...
  /// prose: Percentage values are denoted by <percentage>, and indicates a value that is some fraction of another
  /// reference value.
  Percentage,
  ///
  ...
  /// prose: The <position> value specifies the position of an alignment subject (e.g. a background image) inside an
  /// alignment container (e.g. its background positioning area) as a pair of offsets between the specified edges
  /// (defaulting to the left and top). Its syntax is:
  Position,
  ///
  /// href: https://drafts.csswg.org/css-anchor-position-1/#typedef-position-area
  /// syntax: [ [ left | center | right | span-left | span-right | x-start | x-end | span-x-start | span-x-end |
  /// self-x-start | self-x-end | span-self-x-start | span-self-x-end | span-all ] || [ top | center | bottom | span-top
  /// | span-bottom | y-start | y-end | span-y-start | span-y-end | self-y-start | self-y-end | span-self-y-start |
  /// span-self-y-end | span-all ] | [ block-start | center | block-end | span-block-start | span-block-end | span-all ]
  /// || [ inline-start | center | inline-end | span-inline-start | span-inline-end | span-all ] | [ self-block-start |
  /// center | self-block-end | span-self-block-start | span-self-block-end | span-all ] || [ self-inline-start | center
  /// | self-inline-end | span-self-inline-start | span-self-inline-end | span-all ] | [ start | center | end |
  /// span-start | span-end | span-all ]{1,2} | [ self-start | center | self-end | span-self-start | span-self-end |
  /// span-all ]{1,2} ]
  PositionArea,
  ///
  /// href: https://drafts.csswg.org/css-anchor-position-2/#typedef-position-area-query
  /// syntax: [ [ left | center | right | span-left | span-right | x-start | x-end | span-x-start | span-x-end |
  /// self-x-start | self-x-end | span-self-x-start | span-self-x-end | span-all | any ] || [ top | center | bottom |
  /// span-top | span-bottom | y-start | y-end | span-y-start | span-y-end | self-y-start | self-y-end |
  /// span-self-y-start | span-self-y-end | span-all | any ] | [ block-start | center | block-end | span-block-start |
  /// span-block-end | span-all | any ] || [ inline-start | center | inline-end | span-inline-start | span-inline-end |
  /// span-all | any ] | [ self-block-start | center | self-block-end | span-self-block-start | span-self-block-end |
  /// span-all | any ] || [ self-inline-start | center | self-inline-end | span-self-inline-start | span-self-inline-end
  /// | span-all | any ] | [ start | center | end | span-start | span-end | span-all | any ]{1,2} | [ self-start |
  /// center | self-end | span-self-start | span-self-end | span-all | any ]{1,2} ]
  PositionAreaQuery,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#typedef-position-four
  /// syntax: [ [ [ left | right | x-start | x-end ] <length-percentage> ] && [ [ top | bottom | y-start | y-end ]
  /// <length-percentage> ] | [ [ block-start | block-end ] <length-percentage> ] && [ [ inline-start | inline-end ]
  /// <length-percentage> ] | [ [ start | end ] <length-percentage> ]{2} ]
  PositionFour,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#typedef-position-one
  /// syntax: [ left | center | right | top | bottom | x-start | x-end | y-start | y-end | block-start | block-end |
  /// inline-start | inline-end | <length-percentage> ]
  PositionOne,
  ///
  /// href: https://drafts.csswg.org/css-backgrounds-4/#typedef-position-three
  /// syntax: [ [ left | center | right ] && [ [ top | bottom ] <length-percentage> ] | [ [ left | right ]
  /// <length-percentage> ] && [ top | center | bottom ] ]
  PositionThree,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#typedef-position-two
  /// syntax: [ [ left | center | right | x-start | x-end ] && [ top | center | bottom | y-start | y-end ] | [ left |
  /// center | right | x-start | x-end | <length-percentage> ] [ top | center | bottom | y-start | y-end |
  /// <length-percentage> ] | [ block-start | center | block-end ] && [ inline-start | center | inline-end ] | [ start |
  /// center | end ]{2} ]
  PositionTwo,
  ///
  /// href: https://drafts.csswg.org/css-color-hdr-1/#typedef-predefined-rgb
  /// syntax: srgb | srgb-linear | display-p3 | display-p3-linear | a98-rgb | prophoto-rgb | rec2020 | rec2100-pq |
  ...
  /// prose: The <progress-source> value type represents the interpolation progress in an interpolation notation. Its
  /// syntax is:
  ProgressSource,
  ///
  ...
  /// prose: <qualified-rule-list>: only qualified rules are allowed; declarations and at-rules are automatically
  /// invalid.
  QualifiedRuleList,
  ///
  /// href: https://drafts.csswg.org/css-conditional-5/#typedef-query-in-parens
  /// syntax: ( <container-query> ) | ( <size-feature> ) | style( <style-query> ) | scroll-state( <scroll-state-query> )
  /// | <general-enclosed>
  QueryInParens,
  ///
  /// href: https://drafts.csswg.org/css-color-4/#typedef-quirky-color
  /// prose: When CSS is being parsed in quirks mode, <quirky-color> is a type of <color> that is only valid in certain
  /// properties:
  QuirkyColor,
  ///
  /// href: https://drafts.csswg.org/css-values-4/#typedef-quirky-length
  /// prose: When CSS is being parsed in quirks mode, <quirky-length> is a type of <length> that is only valid in
  /// certain properties:
  QuirkyLength,
  ///
  ...
  /// syntax: [ [ [ <radial-shape> || <radial-size> ]? [ at <position> ]? ] || <color-interpolation-method>]? ,
  /// <color-stop-list>
  ...
  /// prose: Ratio values are denoted by <ratio>, and represent the ratio of two numeric values. It most often
  /// represents an aspect ratio, relating a width (first) to a height (second).
  Ratio,
  ///
  ...
  /// syntax: srgb | srgb-linear | display-p3 | display-p3-linear | a98-rgb | prophoto-rgb | rec2020 | lab | oklab |
  /// <xyz-space>
  RectangularColorSpace,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#typedef-request-url-modifier-referrer-policy-modifier
  /// syntax: referrer-policy(no-referrer | no-referrer-when-downgrade | same-origin | origin | strict-origin |
  /// origin-when-cross-origin | strict-origin-when-cross-origin | unsafe-url) for_parents: [<request-url-modifier>]
  ...
  /// prose: <request-url-modifier>s are <url-modifier>s that affect the <url>’s resource request by applying associated
  /// URL request modifier steps. See CSS Values 4 § 4.5.4 URL Processing Model.
  RequestUrlModifier,
  ///
  ...
  /// prose: If A is exactly equal to an integer multiple of B, round() resolves to A exactly (preserving whether A is
  /// 0⁻ or 0⁺, if relevant). Otherwise, there are two integer multiples of B that are potentially "closest" to A, lower
  /// B which is closer to −∞ and upper B which is closer to +∞. The following <rounding-strategy>s dictate how to
  /// choose between them:
  RoundingStrategy,
  ///
  ...
  /// prose: The four ::scroll-button() pseudo-elements are individually selected by the selector’s argument. A *
  /// arguments selects all four ::scroll-button()s; otherwise the selected pseudo-element is determined by the
  /// <scroll-button-direction> value:
  ScrollButtonDirection,
  ///
  /// href: https://drafts.csswg.org/css-conditional-5/#typedef-scroll-state-feature
  /// prose: A container scroll-state query allows querying a container for state that depends on scroll position. It is
  /// a boolean combination of individual scroll-state features (<scroll-state-feature>) that each query a single
  /// feature of the query container. The syntax of a <scroll-state-feature> is the same as for a media feature: a
  /// feature name, a comparator, and a value.
  ScrollStateFeature,
  ///
  ...
  /// syntax: not <scroll-state-in-parens> | <scroll-state-in-parens> [ [ and <scroll-state-in-parens> ]* | [ or
  /// <scroll-state-in-parens> ]* ] | <scroll-state-feature>
  ScrollStateQuery,
  ///
  ...
  /// prose: When appearing in the first argument, provides any type-specific interpolation options that apply to every
  /// segment in the interpolation map. (For example, color-interpolate() allows <color-interpolation-method>.) When
  /// appearing between stops, provides any type-specific interpolation options that apply to the interpolation segment
  /// between the stops on either side of this argument, overriding any default provided by a corresponding global
  /// <segment-options> argument. for_parents: [calc-interpolate(), color-interpolate(), interpolate(),
  ...
  /// prose: In CSS 2, the only valid <shape> value is: rect(<top>, <right>, <bottom>, <left>) where <top> and <bottom>
  /// specify offsets from the top border edge of the box, and <right>, and <left> specify offsets from the left border
  /// edge of the box. Authors should separate offset values with commas. User agents must support separation with
  /// commas, but may also support separation without commas (but not a combination), because a previous revision of
  /// this specification was ambiguous in this respect. <top>, <right>, <bottom>, and <left> may either have a <length>
  /// value or auto. Negative lengths are permitted. The value auto means that a given edge of the clipping region will
  /// be the same as the edge of the element’s generated border box (i.e., auto means the same as 0 for <top> and
  /// <left>, the same as the used value of the height plus the sum of vertical padding and border widths for <bottom>,
  /// and the same as the used value of the width plus the sum of the horizontal padding and border widths for <right>,
  /// such that four auto values result in the clipping region being the same as the element’s border box). When
  /// coordinates are rounded to pixel coordinates, care should be taken that no pixels remain visible when <left> and
  /// <right> have the same value (or <top> and <bottom> have the same value), and conversely that no pixels within the
  ...
  /// syntax: <move-command> | <line-command> | close | <horizontal-line-command> | <vertical-line-command> |
  /// <curve-command> | <smooth-command> | <arc-command> prose: The sequence of <shape-command>s represent further
  /// path data commands. Each command’s starting point is the previous command’s ending point.
  ShapeCommand,
  ///
  ...
  /// syntax: <'animation-duration'> || <easing-function> || <'animation-delay'> || <single-animation-iteration-count>
  /// || <single-animation-direction> || <single-animation-fill-mode> || <single-animation-play-state> || [ none |
  /// <keyframes-name> ] || <single-animation-timeline>
  SingleAnimation,
  ///
  ...
  /// prose: A container size query allows querying the size of the query container’s principal box. It is a boolean
  /// combination of individual size features (<size-feature>) that each query a single, specific dimensional feature of
  /// the query container. The syntax of a <size-feature> is the same as for a media feature: a feature name, a
  /// comparator, and a value. [mediaqueries-5] The boolean syntax and logic combining size features into a size query
  /// is the same as for CSS feature queries. (See @supports. [css-conditional-3])
  SizeFeature,
  ///
  /// href: https://drafts.csswg.org/css-values-5/#typedef-size-keyword
  /// prose: The <size-keyword> production matches any sizing keywords allowed in the context. For example, in width, it
  /// matches auto, min-content, stretch, etc.
  SizeKeyword,
  ///
  ...
  /// syntax: smooth [ [ to <position> [ with <control-point> ]? ] | [ by <coordinate-pair> [ with
  /// <relative-control-point> ]? ] ] for_parents: [shape()]
  SmoothCommand,
  ///
  ...
  /// syntax: <'box-shadow-color'>? && [ [ none | <length>{2} ] [ <'box-shadow-blur'> <'box-shadow-spread'>? ]? ] &&
  /// <'box-shadow-position'>?
  SpreadShadow,
  ///
  ...
  /// prose: Strings are denoted by <string>. When written literally, they consist of a sequence of characters delimited
  /// by double quotes or single quotes, corresponding to the <string-token> production in the CSS Syntax Module
  /// [CSS-SYNTAX-3].
  String,
  ///
  ...
  /// prose: A container style query allows querying the computed values of the query container. It is a boolean
  /// combination of individual style features (<style-feature>) that each query a single, specific property of the
  /// query container. The syntax of a <style-feature> is either the same as for a valid declaration[CSS-SYNTAX-3], a
  /// <style-feature-name> or a valid style range (<style-range>). The <style-feature-name> can be either a supported
  /// CSS property or a valid <custom-property-name>. The <style-feature-value> production matches any valid
  /// <declaration-value> as long as it doesn’t contain <mf-lt>, <mf-gt> and <mf-eq> tokens.
  StyleFeatureName,
  ///
  ...
  /// syntax: <style-range-value> <mf-comparison> <style-range-value> | <style-range-value> <mf-lt> <style-range-value>
  /// <mf-lt> <style-range-value> | <style-range-value> <mf-gt> <style-range-value> <mf-gt> <style-range-value>
  StyleRange,
  ///
  ...
  /// syntax: not <supports-in-parens> | <supports-in-parens> [ and <supports-in-parens> ]* | <supports-in-parens> [ or
  /// <supports-in-parens> ]*
  SupportsCondition,
  ///
  ...
  /// syntax: <supports-selector-fn> | <supports-font-tech-fn> | <supports-font-format-fn> | <supports-at-rule-fn> |
  /// <supports-named-feature-fn> | <supports-env-fn> | <supports-decl>
  SupportsFeature,
  ///
  ...
  /// syntax: angle | color | custom-ident | image | integer | length | length-percentage | number | percentage |
  /// resolution | string | time | url | transform-function
  SyntaxTypeName,
  ///
  /// href: https://drafts.csswg.org/css-color-4/#typedef-system-color
  /// syntax: AccentColor | AccentColorText | ActiveText | ButtonBorder | ButtonFace | ButtonText | Canvas | CanvasText
  /// | Field | FieldText | GrayText | Highlight | HighlightText | LinkText | Mark | MarkText | SelectedItem |
  /// SelectedItemText | VisitedText | <deprecated-color> prose: However, in forced colors mode, most colors on the
  /// page are forced into a restricted, user-chosen palette, see CSS Color Adjustment 1 § 5.2 Forced Colors Mode Color
  /// Palettes. The <system-color> keywords expose these user-chosen colors so that the rest of the page can integrate
  /// with this restricted palette.
  SystemColor,
  ///
  ...
  /// prose: A locally installed system font, whose use is subject to certain constraints. In particular, it may not be
  /// used with the font-family property, but can be used with the font shorthand. The following values refer to system
  /// fonts:
  ...
  /// syntax: <wcag2>
  /// prose: The <target-contrast> argument specifies the contrast algorithm(s) to use. If no color candidates have been
  /// provided, <target-contrast> may be omitted, in which case a UA-chosen algorithm is used.
  TargetContrast,
  ///
  /// href: https://drafts.csswg.org/css-ui-4/#typedef-target-name
  /// prose: The <target-name> parameter indicates the target frame for the focus navigation. It is a <string> and it
  /// MUST NOT start with the underscore "_" character. Error handling: if it does start with an underscore, "_parent"
  /// navigates to the parent frame, "_root" is treated as root, and other values navigate to a frame by that name if it
  /// exists. If the specified target frame does not exist, the parameter will be treated as the keyword current, which
  /// means to simply use the frame that the element is in. The keyword root indicates that the user agent should target
  /// the full window.
  TargetName,
  ///
  /// href: https://drafts.csswg.org/css-inline-3/#typedef-text-edge
  /// syntax: [ text | ideographic | ideographic-ink ] | [ text | ideographic | ideographic-ink | cap | ex ] [ text |
  /// ideographic | ideographic-ink | alphabetic ] prose: The <text-edge> value, which identifies specific font
  /// metrics, expands to
  TextEdge,
  ///
  ...
  /// prose: The animation range center subject is the element whose principal box is the range relative to which the
  /// center of the active range is calculated, and is represented by the <timeline-range-center-subject> value type,
  /// which indicates a CSS identifier representing one of the following:
  TimelineRangeCenterSubject,
  ///
  /// href: https://drafts.csswg.org/scroll-animations-1/#typedef-timeline-range-name
  /// prose: A named timeline range is a named segment of an animation timeline. The start of the segment is represented
  /// as 0% progress through the range; the end of the segment is represented as 100% progress through the range.
  /// Multiple named timeline ranges can be associated with a given timeline, and multiple such ranges can overlap. For
  /// example, the contain range of a view progress timeline overlaps with its cover range. Named timeline ranges are
  /// represented by the <timeline-range-name> value type, which indicates a CSS identifier representing one of the
  ...
  /// syntax: <scale3d()> | <scale()> | <scaleX()> | <scaleY()> | <scaleZ()> | <translate3d()> | <translate()> |
  /// <translateX()> | <translateY()> | <translateZ()> | <rotate3d()> | <rotate()> | <rotateX()> | <rotateY()> |
  /// <rotateZ()> | <skew()> | <skewX()> | <skewY()> | <matrix3d()> | <matrix()> | <perspective()> prose: The value of
  /// the transform property is a list of <transform-function>. The set of allowed transform functions is given below.
  /// Wherever <angle> is used in this specification, a <number> that is equal to zero is also allowed, which is treated
  /// the same as an angle of zero degrees. A percentage for horizontal translations is relative to the width of the
  /// reference box. A percentage for vertical translations is relative to the height of the reference box. A percentage
  /// in a scale function is equivalent to a number, and serializes as a number in specified values. For example,
  /// scale3d(50%, 100%, 150%) serializes as scale3d(0.5, 1, 1.5).
  TransformFunction,
  ///
  ...
  /// prose: URI values (Uniform Resource Identifiers, see [RFC3986], which includes URLs, URNs, etc) in this
  /// specification are denoted by <uri>. The functional notation used to designate URIs in property values is "url()",
  /// as in:
  Uri,
  ///
  ...
  /// prose: <url>s support specifying additional <url-modifier>s, which change the meaning or the interpretation of the
  /// URL somehow. A <url-modifier> is either an <ident> or a functional notation.
  UrlModifier,
  ///
  /// href: https://drafts.csswg.org/css-ui-4/#typedef-cursor-url-set
  /// prose: <url-set> is a limited version of image-set(), where the <image> sub-production is restricted to <url>
  /// only. for_parents: [cursor]
  UrlSet,
  ///
  ...
  /// prose: Values are specific voice instances (e.g., Mike, comedian, mary, carlos2, "valley girl"). Like font-family
  /// names, voice names must either be given quoted as strings, or unquoted as a sequence of one or more CSS
  /// identifiers. If a sequence of identifiers is given as a voice name, the computed value is the name converted to a
  /// string by joining all the identifiers in the sequence by single spaces. Voice names that happen to be the same as
  /// the gender keywords (male, female and neutral) or that happen to match the CSS-wide keywords or preserve must be
  /// quoted to disambiguate with these keywords. The keyword default is reserved for future use and must also be quoted
  /// when used as voice names. It is recommended to quote voice names that contain white space, digits, or punctuation
  /// characters other than hyphens—​even if these voice names are valid in unquoted form—​in order to improve code
  /// clarity. For example: voice-family: "john doe", "Henry the-8th"; for_parents: [voice-family]
  VoiceFamilyName,
  ///
  ...
  /// prose: The value <zero> represents a literal number with the value 0. Expressions that merely evaluate to a
  /// <number> with the value 0 (for example, calc(0)) do not match <zero>; only literal <number-token>s do.
  Zero,
  Unknown(String),
}
  ...
  pub fn from_name(name: &str) -> Self {
    const ENTRIES: &[(&str, CssType)] = &[
      ...
      (
        ...
        CssType::NavigationLocationBetweenTest,
      ),
      ...
      (
        ...
        CssType::SingleAnimationIterationCount,
      ),
      ...
      (
        ...
        CssType::SlashSeparatedBorderRadiusSyntax,
      ),
