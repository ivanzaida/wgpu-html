...
use wgpu_html_winit::{
  AppHook, EventResponse, FrameTimings, HookContext, WgpuHtmlWindow, create_window, register_system_fonts,
      ...
      if let PhysicalKey::Code(code) = event.physical_key {
        match code {
          KeyCode::F9 => {
            ...
            println!("demo: profiling {}", if self.enabled { "enabled" } else { "disabled" });
            if !self.enabled {
              self.profiler.reset();
            }
