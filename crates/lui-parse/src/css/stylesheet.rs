use lui_core::{AtRule, CssAtRule, Declaration, ParseError, SelectorList, StyleRule, Stylesheet};
...
fn parse_rule_list(
  input: &str,
  inside_at_rule: bool,
  at_rule_name: Option<&str>,
) -> Result<(Stylesheet, usize), ParseError> {
  ...
  for part in input.split(';') {
    ...
    if let Some((prop, val)) = part.split_once(':') {
      ...
      match parse_declaration(prop, val_str) {
        Ok((property, value)) => decls.push(Declaration {
          property,
          value,
          important,
        }),
...
#[cfg(test)]
mod tests {
  use lui_core::{CssProperty, CssPseudo};
