use lui_core::{ArcStr, color::CssColor, css_property::CssProperty, error::ParseError, value::CssValue};

use crate::css::tokenizer::{Token, tokenize};
...
fn parse_url_function(tokens: &[Token], pos: usize) -> Result<(CssValue, usize), ParseError> {
  ...
  let url: ArcStr = tokens[url_start..p]
    .iter()
    .map(|t| match t {
    ...
    Token::Ident(s) | Token::String(s) => {
      if !s.starts_with("--") {
        return Err(ParseError::new(
          format!("var() expects a custom property starting with --, got {s}"),
          p,
        ));
      }
      p += 1;
