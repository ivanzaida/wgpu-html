...
impl Component for Counter {
  ...
  fn styles() -> style::Stylesheet {
    style::sheet([
      ...
      style::rule(".btn:hover").background_color("#4A4B4F"),
  ...
  fn view(&self, props: &CounterProps, ctx: &Ctx<CounterMsg>, _env: &()) -> El {
    el::div().class(ctx.scoped("root")).children([
      ...
      el::div().class(ctx.scoped("value")).text(self.count.to_string()),
  ...
  fn view(&self, _props: &DemoProps, ctx: &Ctx<DemoMsg>, _env: &()) -> El {
    el::div().class(ctx.scoped("root")).children([
      ...
      el::div().class(ctx.scoped("counters")).children([
        ctx.child::<Counter>(CounterProps { label: "Clicks".into() }),
        ctx.child::<Counter>(CounterProps { label: "Score".into() }),
      ]),
    ])
  }
}
...
fn main() {
  App::new::<DemoApp>(DemoProps)
    ...
    .size(800, 500)
    .stylesheet(
      "html, body { height: 100%; margin: 0; background: #1a1a1a; } body { display: flex; flex-direction: column; }",
    )
