...
fn auto_close_p_before_p() {
    ...
    assert_eq!(doc.root.children[0].element, HtmlElement::P);
    assert_eq!(doc.root.children[0].children.len(), 1);
    assert_eq!(text_of(&doc.root.children[0].children[0]), "one");
    assert_eq!(doc.root.children[1].element, HtmlElement::P);
    assert_eq!(doc.root.children[1].children.len(), 1);
    assert_eq!(text_of(&doc.root.children[1].children[0]), "two");
}
...
fn comments_and_doctype_dropped() {
    ...
    assert_eq!(doc.root.children.len(), 2);
    assert_eq!(doc.root.children[0].element, HtmlElement::Comment(ArcStr::from("c")));
    let p = &doc.root.children[1];
    assert_eq!(p.element, HtmlElement::P);
    assert_eq!(p.children.len(), 1);
    assert!(matches!(p.children[0].element, HtmlElement::Text(_)));
...
fn consecutive_body_elements() {
    ...
    assert_eq!(doc.root.children.len(), 2);
    assert_eq!(doc.root.children[0].element, HtmlElement::Body);
    assert_eq!(doc.root.children[0].children.len(), 1);
    assert_eq!(doc.root.children[0].children[0].element, HtmlElement::P);
    assert_eq!(doc.root.children[1].element, HtmlElement::Body);
    assert_eq!(doc.root.children[1].children.len(), 1);
    assert_eq!(doc.root.children[1].children[0].element, HtmlElement::P);
}
...
fn style_and_body_are_siblings() {
    ...
    assert_eq!(doc.root.children.len(), 2);
    assert_eq!(doc.root.children[0].element, HtmlElement::Style);
    assert_eq!(doc.root.children[0].children.len(), 1);
    assert!(matches!(doc.root.children[0].children[0].element, HtmlElement::Text(_)));
    assert_eq!(doc.root.children[1].element, HtmlElement::Body);
    assert_eq!(doc.root.children[1].children[0].element, HtmlElement::P);
}
...
fn consecutive_html_elements() {
    ...
    assert_eq!(doc.root.element, HtmlElement::Html);
    assert_eq!(doc.root.children.len(), 2);
    assert_eq!(doc.root.children[0].element, HtmlElement::Html);
    assert_eq!(doc.root.children[0].children.len(), 1);
    assert_eq!(doc.root.children[0].children[0].element, HtmlElement::Body);
    assert_eq!(doc.root.children[1].element, HtmlElement::Html);
    assert!(doc.root.children[1].children.is_empty());
}
...
#[test]
fn pure_text_input_becomes_child() {
    let doc = parse("hello");
    assert_eq!(doc.root.children.len(), 1);
    assert_eq!(text_of(&doc.root.children[0]), "hello");
...
fn comment_only() {
    ...
    assert_eq!(doc.root.children.len(), 1);
    assert_eq!(doc.root.children[0].element, HtmlElement::Comment(ArcStr::from(" nothing here ")));
}
...
fn span_with_text() {
    ...
    assert_eq!(doc.root.children.len(), 1);
    assert_eq!(doc.root.children[0].element, HtmlElement::Span);
    assert_eq!(doc.root.children[0].children.len(), 1);
}
...
#[test]
fn multiple_top_level_become_children() {
...
fn select_with_options() {
    ...
    let select = &doc.root.children[0];
...
fn p_auto_closes_when_div_opens() {
    ...
    assert_eq!(doc.root.children[0].element, HtmlElement::P);
    assert_eq!(doc.root.children[0].children.len(), 1);
    assert_eq!(text_of(&doc.root.children[0].children[0]), "a");
    assert_eq!(doc.root.children[1].element, HtmlElement::Div);
    assert_eq!(text_of(&doc.root.children[1].children[0]), "b");
}
...
fn doctype_and_comments_are_stripped() {
    ...
    assert_eq!(doc.root.children[0].element, HtmlElement::Comment(ArcStr::from("top")));
...
#[test]
fn comment_preserved_as_child_with_div() {
    let doc = parse("<!-- hello --><div></div>");
    assert_eq!(doc.root.children.len(), 2);
    assert_eq!(doc.root.children[0].element, HtmlElement::Comment(" hello ".into()));
    assert_eq!(doc.root.children[1].element, HtmlElement::Div);
}
...
fn custom_element_with_dash() {
    ...
    assert_eq!(doc.root.children.len(), 1);
    assert_eq!(doc.root.children[0].element, HtmlElement::Unknown("my-widget".into()));
    assert_eq!(doc.root.children[0].attrs.get("foo").map(|s| &**s), Some("bar"));
    assert_eq!(doc.root.children[0].children.len(), 1);
    assert_eq!(doc.root.children[0].children[0].element, HtmlElement::Text("content".into()));
}
...
fn raw_text_script_parses_content_as_text() {
    ...
    assert_eq!(doc.root.children.len(), 1);
    assert_eq!(doc.root.children[0].element, HtmlElement::Script);
    assert_eq!(doc.root.children[0].children.len(), 1);
    assert_eq!(doc.root.children[0].children[0].element, HtmlElement::Text("var x = '<div>';".into()));
}
...
fn raw_text_style_parses_content_as_text() {
    ...
    assert_eq!(doc.root.children.len(), 1);
    assert_eq!(doc.root.children[0].element, HtmlElement::Style);
    assert_eq!(doc.root.children[0].children[0].element, HtmlElement::Text("div { color: red; }".into()));
}
...
fn all_known_elements_parse() {
    for tag in &["html", "head", "body", "title", "base", "link", "meta", "style",
        ...
        if *tag == "html" {
            ...
            assert_eq!(doc.root.element, HtmlElement::Html, "<html> should be root");
            continue;
        }
        let html = format!("<{}></{}>", tag, tag);
        let doc = parse(&html);
        let element = HtmlElement::from_name(tag);
        if element.is_void() {
            let doc2 = parse(&format!("<{}>", tag));
            assert!(!doc2.root.children.is_empty(), "void element <{}> should parse", tag);
        } else {
            assert!(!doc.root.children.is_empty(), "<{}> should parse", tag);
        }
    }
}

#[test]
fn html_root_always_present() {
    let doc = parse("");
    assert_eq!(doc.root.element, HtmlElement::Html);
...
#[test]
fn html_tag_not_duplicated() {
    let doc = parse("<html><body><p>ok</p></body></html>");
    assert_eq!(doc.root.element, HtmlElement::Html);
    assert_eq!(doc.root.children.len(), 1);
    assert_eq!(doc.root.children[0].element, HtmlElement::Body);
}
