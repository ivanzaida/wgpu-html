...
mod tests {
    ...
    fn attrs() {
        assert_eq!(
            tokenize(r#"<div id="x" class="y" hidden>"#),
            vec![Token::OpenTag {
                name: "div".into(),
                attrs: vec![
                    ("id".into(), "x".into()),
    ...
    fn raw_text_script() {
        ...
        assert!(matches!(tokens[0], Token::OpenTag { ref name, .. } if name == "script"));
        assert_eq!(tokens[1], Token::Text("a < b".into()));
        assert!(matches!(tokens[2], Token::CloseTag(ref name) if name == "script"));
    }
