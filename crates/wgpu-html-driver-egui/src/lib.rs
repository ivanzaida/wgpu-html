//! egui integration for [`wgpu_html_driver`].
//!
...
use egui::{
    Align2, Color32, Event, FontId, PointerButton, Pos2, Response, Sense, Ui, Vec2,
    Rect as EguiRect,
};
...
impl<W: HasWindowHandle + HasDisplayHandle + Send + Sync + 'static> Driver for EguiHtml<W> {
    ...
    fn set_cursor(&self, _cursor: wgpu_html::layout::Cursor) {}
}
...
impl<W: HasWindowHandle + HasDisplayHandle + Send + Sync + 'static> EguiRunner<W> {
    ...
    pub fn show(&mut self, ui: &mut Ui, tree: &mut Tree, desired_size: Vec2) -> HtmlOutput {
        ...
        self.rt.driver.set_viewport(
            ((rect.width() * scale).max(1.0) as u32, (rect.height() * scale).max(1.0) as u32),
            scale as f64,
        );
