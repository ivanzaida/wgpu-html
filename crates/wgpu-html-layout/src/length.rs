use wgpu_html_models::common::css_enums::{CssLength, CssMathExpr, CssNumericFunction};
...
fn eval_math(expr: &CssMathExpr, parent_size_px: f32, ctx: &Ctx) -> Option<MathValue> {
    match expr {
        CssMathExpr::Length(len) => {
            resolve(Some(len), parent_size_px, ctx).map(MathValue::LengthPx)
        }
        ...
        CssMathExpr::Mul(a, b) => {
            ...
            match (a, b) {
                (MathValue::LengthPx(x), MathValue::Number(y))
                | (MathValue::Number(y), MathValue::LengthPx(x)) => {
        ...
        CssMathExpr::Div(a, b) => {
            ...
            match (a, b) {
                (MathValue::LengthPx(x), MathValue::Number(_)) => {
                    Some(MathValue::LengthPx(x / denom))
                }
                (MathValue::Number(x), MathValue::Number(_)) => Some(MathValue::Number(x / denom)),
                (MathValue::LengthPx(x), MathValue::LengthPx(_)) => {
                    Some(MathValue::Number(x / denom))
                }
                (MathValue::Number(_), MathValue::LengthPx(_)) => None,
            }
        }
        CssMathExpr::Function(kind, args) => {
            eval_numeric_function(*kind, args, parent_size_px, ctx)
        }
    }
}
...
fn eval_numeric_function(
    ...
    let v = match kind {
        ...
        CssNumericFunction::Mod | CssNumericFunction::Rem => {
            nums.first()?.rem_euclid(*nums.get(1)?)
        }
