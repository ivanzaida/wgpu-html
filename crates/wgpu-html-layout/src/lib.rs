...
fn sized_cache() -> &'static Mutex<HashMap<SizedKey, CacheEntry<Option<ImageData>>>> {
  static CACHE: OnceLock<Mutex<HashMap<SizedKey, CacheEntry<Option<ImageData>>>>> = OnceLock::new();
  CACHE.get_or_init(|| Mutex::new(HashMap::new()))
}
...
pub fn sweep_image_cache() {
  ...
  if let Ok(mut cache) = raw_cache().lock() {
    cache.retain(|_, entry| {
      ...
      matches!(entry.value, RawState::Pending) || now.duration_since(entry.last_access) < entry.effective_ttl(ttl)
    });
  }
}
...
fn raw_state_bytes(state: &RawState) -> usize {
  match state {
    ...
    RawState::Ready(DecodedAsset::Animated { frames, .. }) => frames.iter().map(|f| f.rgba.len()).sum(),
  }
}
...
impl ImageCache {
  ...
  pub fn load_url(&mut self, src: &str, declared_w: Option<u32>, declared_h: Option<u32>) -> Option<ImageData> {
    self.maybe_sweep();
...
fn build_sized(src: &str, asset: &DecodedAsset, declared_w: Option<u32>, declared_h: Option<u32>) -> Option<ImageData> {
  match asset {
...
/// immediately:
///
/// - Before the pool has been initialised: changes the count used on first init.
/// - After init: spawns additional workers if `n` exceeds the current count. **Shrinking is a no-op** — workers are
///   never killed once started.
///
...
pub(crate) fn load_image_url(src: &str, declared_w: Option<u32>, declared_h: Option<u32>) -> Option<ImageData> {
