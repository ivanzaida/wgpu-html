...
fn full_and_incremental(
  html: &str,
  mutate: impl FnOnce(&mut wgpu_html_tree::Tree),
  dirty: &[Vec<usize>],
  vw: f32,
  vh: f32,
) -> (LayoutBox, LayoutBox) {
  ...
  mutate(&mut tree);
  let cascaded2 = wgpu_html_style::cascade(&tree);

  let mut text_ctx2 = wgpu_html_text::TextContext::new(64);
  ...
  let mut text_ctx3 = wgpu_html_text::TextContext::new(64);
