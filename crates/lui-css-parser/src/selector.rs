...
pub fn parse_selector_list(input: &str) -> Result<SelectorList, ParseError> {
    ...
    if pos < chars.len() {
        return Err(ParseError { message: format!("unexpected '{}' at {}", chars[pos], pos), position: pos });
    }
...
fn parse_combinator(chars: &[char], pos: &mut usize) -> Option<CssCombinator> {
    ...
    match chars[*pos] {
        '>' => { *pos += 1; Some(CssCombinator::Child) }
        '+' => { *pos += 1; Some(CssCombinator::NextSibling) }
        '~' => { *pos += 1; Some(CssCombinator::SubsequentSibling) }
        '|' if *pos + 1 < chars.len() && chars[*pos + 1] == '|' => {
            *pos += 2; Some(CssCombinator::Column)
        }
        _ => {
...
fn parse_compound_selector(chars: &[char], pos: &mut usize) -> Result<CompoundSelector, ParseError> {
    ...
    if *pos == start {
        return Err(ParseError { message: "empty compound selector".to_string(), position: *pos });
    }
    ...
    *pos += 1;
    ...
    let arg = if *pos < chars.len() && chars[*pos] == '(' {
        *pos += 1;
        ...
        if depth != 0 {
            return Err(ParseError { message: "unclosed '(' in pseudo".to_string(), position: start });
        }
...
fn parse_attr(chars: &[char], pos: &mut usize) -> Result<AttributeSelector, ParseError> {
    *pos += 1; // skip '['
    ...
    skip_ws(chars, pos);
    if *pos >= chars.len() || chars[*pos] != ']' {
        return Err(ParseError { message: "expected ']'".to_string(), position: *pos });
    }
    *pos += 1;

    Ok(AttributeSelector { name, op, value, modifier })
}
