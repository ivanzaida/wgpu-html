...
The layout engine (`lui-layout-old`) converts a fully-cascaded style tree into a **`LayoutBox`** tree — a positioned, sized, shaped representation of every element in physical pixels. The renderer consumes `LayoutBox` directly; it never re-resolves CSS.
