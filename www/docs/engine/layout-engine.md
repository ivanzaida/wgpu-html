...
The layout engine converts the cascaded style tree into positioned boxes with pixel coordinates. It lives in `crates/wgpu-html-layout/src/lib.rs`.
...
Implements CSS Flexbox Level 1 (`crates/wgpu-html-layout/src/flex.rs`):
...
Implements CSS Grid Layout Level 1 (`crates/wgpu-html-layout/src/grid.rs`):
...
CSS lengths (`px`, `%`, `em`, `rem`, `vw`, `vh`, `vmin`, `vmax`, `calc()`, `min()`, `max()`, `clamp()`) are resolved to pixels via `crates/wgpu-html-layout/src/length.rs`. `auto` returns `None` and the caller decides the behavior.
