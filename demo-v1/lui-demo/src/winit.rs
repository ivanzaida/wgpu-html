...
fn write_cascaded_json(out: &mut String, node: &lui_style::CascadedNode, depth: usize) {
  use std::fmt::Write;
...
fn write_attrs(out: &mut String, el: &lui_tree::Element, indent: &str) {
  use std::fmt::Write;
  let names = [
    "type",
    "name",
...
fn write_computed_style(out: &mut String, s: &lui_models::Style, indent: &str) {
  use std::fmt::Write;
  ...
  fn run_command(&mut self, cmd: DemoCommand) {
    match cmd {
      ...
      DemoCommand::DumpTree { selector } => {
        let cascaded = lui_style::cascade(&self.driver.tree);
...
pub(crate) fn run(mut tree: Tree, doc_source: String, profiling_enabled: bool) -> ExitCode {
  ...
  let attrs = winit::window::Window::default_attributes()
    .with_title(format!("lui demo: {doc_source}"))
