...
use lui_parser::{ComplexSelector, Stylesheet, parse_stylesheet};

fn one_rule_selector(sheet: &Stylesheet) -> &ComplexSelector {
  &sheet.rules[0].selectors[0]
}

fn first_compound(sel: &ComplexSelector) -> &lui_parser::CompoundSelector {
  &sel.compounds[0]
}
...
fn child_combinator_now_supported() {
  ...
  assert!(matches!(sels[0].combinators[0], lui_parser::Combinator::Child));
...
fn parse_import_directive_double_quoted_url() {
  use lui_parser::parse_import_directive;
