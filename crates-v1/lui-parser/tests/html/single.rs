//! One test per HTML element. Every supported tag must parse to its
//! corresponding `Element` enum variant.
...
/// Non-void tags get `<tag></tag>`; void tags get `<tag>` (no closer).
