...
fn border_no_explicit_color_uses_foreground() {
  let tree = make(
    r#"<body style="margin: 0; width: 50px; height: 50px;
                    color: red; border: 2px solid;"></body>"#,
  );
  let root = layout(&tree, 800.0, 600.0).unwrap();
  assert_eq!(root.border.top, 2.0);
  assert_eq!(root.border_colors.top, Some(LINEAR_RED));
  assert_eq!(root.border_colors.right, Some(LINEAR_RED));
  assert_eq!(root.border_colors.bottom, Some(LINEAR_RED));
  assert_eq!(root.border_colors.left, Some(LINEAR_RED));
}
...
fn border_color_currentcolor_keyword_resolves() {
  let tree = make(
    r#"<body style="margin: 0; width: 50px; height: 50px;
                    color: blue; border: 2px solid currentColor;"></body>"#,
  );
  let root = layout(&tree, 800.0, 600.0).unwrap();
  assert_eq!(root.border_colors.top, Some(LINEAR_BLUE));
  assert_eq!(root.border_colors.left, Some(LINEAR_BLUE));
}

#[test]
fn border_color_inherits_from_parent_color() {
  let root = layout_with_fonts(
    r#"<body style="margin: 0; color: red;">
         <div style="width: 50px; height: 50px; border: 1px solid;"></div>
       </body>"#,
    800.0,
    600.0,
  );
...
fn background_color_currentcolor_resolves() {
  let tree = make(
    r#"<body style="margin: 0; width: 50px; height: 50px;
                    color: red; background-color: currentColor;"></body>"#,
  );
  let root = layout(&tree, 800.0, 600.0).unwrap();
  assert_eq!(root.background, Some(LINEAR_RED));
}
...
fn border_without_color_defaults_to_black_when_no_color_property() {
  ...
  let root = layout(&tree, 800.0, 600.0).unwrap();
  assert_eq!(root.border.top, 2.0);
  assert_eq!(root.border_colors.top, Some(BLACK));
}
  ...
  let root = layout_with_fonts(
    r#"<body style="margin: 0; color: red;">
         <div style="color: blue;">
           <div style="width: 40px; height: 40px; border: 1px solid;"></div>
         </div>
       </body>"#,
    800.0,
    600.0,
