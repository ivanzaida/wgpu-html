...
use lui_tree::Tree;
use lui_ui::{Component, Ctx, El, MsgSender, ShouldRender, el};
...
fn bootstrap<C: Component>(props: C::Props) -> (lui_ui::core::runtime::Runtime, Tree)
where
  C::Msg: Clone + Send + Sync + 'static,
  C::Props: 'static,
{
  let wake: Arc<dyn Fn() + Send + Sync> = Arc::new(|| {});
  let mut rt = lui_ui::core::runtime::Runtime::new::<C>(&props, wake);
