use lui_models as m;
use lui_tree::{Element, Node, Tree, wrap_in_document};
...
fn wrap_in_document_preserves_html_root() {
  let html =
    Node::new(m::Html::default()).with_children(vec![Node::new(m::Head::default()), Node::new(m::Body::default())]);
...
fn wrap_in_document_preserves_body_root_inside_html() {
  let body = Node::new(m::Body::default()).with_children(vec![Node::new(m::Div::default())]);
...
fn set_root_preserves_children_of_wrapped_node() {
  ...
  let div =
    Node::new(m::Div::default()).with_children(vec![Node::new(inner), Node::new(Element::Text("hello".into()))]);
  tree.set_root(div);
