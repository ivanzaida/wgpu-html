...
fn box_shadow_emits_quad_before_background() {
  ...
  assert!(
    list.quads.len() >= 2,
    "expected shadow + background quads, got {}",
    list.quads.len()
  );
  ...
  assert!(
    shadow.shadow_sigma > 0.0,
    "shadow sigma should be > 0, got {}",
    shadow.shadow_sigma
  );
...
fn box_shadow_offset_shifts_position() {
  ...
  assert!(
    approx(shadow.rect.x - bg.rect.x, 10.0),
    "x offset = 10: got {}",
    shadow.rect.x - bg.rect.x
  );
  assert!(
    approx(shadow.rect.y - bg.rect.y, 20.0),
    "y offset = 20: got {}",
    shadow.rect.y - bg.rect.y
  );
}
...
fn box_shadow_blur_expands_and_sets_sigma() {
  ...
  let shadow = &list.quads[0];
  assert!(
    approx(shadow.shadow_sigma, 20.0),
    "sigma = blur = 20: got {}",
    shadow.shadow_sigma
  );
  // Rect expanded by blur on each side
  assert!(
    approx(shadow.rect.w, 140.0),
    "w = 100 + 2*20 = 140: got {}",
    shadow.rect.w
  );
}
...
fn shadow_respects_border_radius() {
  ...
  assert!(
    shadow.radii_h[0] > 8.0,
    "shadow radius > box radius: got {}",
    shadow.radii_h[0]
  );
}
